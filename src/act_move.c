...
void move_char( CHAR_DATA *ch, int door, bool follow )
{
  ...
  if ( !IS_AFFECTED(ch, AFF_SNEAK) && ch->invis_level < LEVEL_HERO) {
    ...
    if (ch->invis_level < LEVEL_HERO)  {
      ...
      for ( fch = in_room->people; fch != NULL; fch = fch_next ) {
	   ...
	   if (IS_SET(fch->merits, MERIT_ACUTESENSES)) {
		chance *= 2;
	   }
  ...
  // Recall timer
  if (!IS_NPC(ch)) {
  	ch->pcdata->next_recall = current_time + 30;
  }
    ...
    for (pWard = ch->in_room->wards; pWard != NULL; pWard = pWard->next) {
	 ...
	 if (fch != NULL) {
	   if (ch->in_room != fch->in_room) {
		send_to_char("A bird chirps and breaks the silence for a few moments.\n\r", fch);
		act("A bird chirps and breaks the silence for a few moments.", fch, NULL, NULL, TO_ROOM);
	   }
	   act("A bird chirps and breaks the silence for a few moments.", ch, NULL, NULL, TO_CHAR);	   
	   act("A bird chirps and breaks the silence for a few moments.", ch, NULL, NULL, TO_ROOM);	   
	 }
    }
  }
  ...
  if (IS_SET(ch->in_room->room_flags,ROOM_STEDDING) && IS_AFFECTED(ch, AFF_CHANNELING)) {
    send_to_char("\n{CYou feel a sudden chill as you walk into this room, then the One Power is no longer in reach.{x\n\r", ch);
...
void do_sneak( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("Your movements become more stealthy.\n\r", ch);
...
void do_visible( CHAR_DATA *ch, char *argument )
{
  ...
  if (IS_SET(ch->affected_by, AFF_INVISIBLE)) {
    ...
    act("$n appears, as if from nowhere.", ch, NULL, NULL, TO_ROOM);
  }
...
void do_recall( CHAR_DATA *ch, char *argument )
{
  ...
  if (IS_AFFECTED(ch, AFF_BIND)) {
    send_to_char("You are tied up and unable to move!\n\r", ch);
    act("$n struggles against the ropes around $s body.", ch, NULL, NULL, TO_ROOM);
    return;
  }
  ...
  if (IS_AFFECTED(ch, AFF_WRAPPED)) {
    send_to_char("{WAn unseen force prevents you from moving!{x\n\r", ch);
    act("$n struggles against unseen bonds.", ch, NULL, NULL, TO_ROOM);
    return;
  }
  ...
  if (!IS_NPC(ch)) {
    if (IS_SET(ch->act, PLR_GRADUATED)) 
    {
	 ...
	 if (!str_prefix(argument,"whitetower"))
	 {
		location = get_room_index(ROOM_VNUM_RECALL_WHITETOWER );
	 }
	 else
	 if (!str_prefix(argument,"wt"))
	 {
		location = get_room_index(ROOM_VNUM_RECALL_WHITETOWER );
	 }
...
void do_bash_door( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_SET(pexit->exit_info, EX_FIREWALL)) {
	  send_to_char("You suddenly notice that the door is very hot to the touch!!\n\r", ch);
	  damage(ch, ch, UMAX(100, ch->hit/2), gsn_wof, DAM_FIRE, FALSE);
	  return;
	}
    ...
    */
	...
	send_to_char( "You succeeded in breaking the door.\n\r", ch );
	...
	&&   pexit_rev->u1.to_room == ch->in_room )
	{
	    ...
	    REMOVE_BIT( pexit_rev->exit_info, EX_LOCKED );
	    for ( rch = to_room->people; rch != NULL; rch = rch->next_in_room )
		act( "The $d crashes opens.", rch, NULL, pexit_rev->keyword, TO_CHAR );
	}
...
void do_push( CHAR_DATA *ch, char *argument )
{
  ...
  if ( arg1[0] == '\0' || arg2[0] == '\0') {
    send_to_char( "Push whom in what diretion?\n\r", ch );
    return;
  }
...
int send_arrow( CHAR_DATA *ch, CHAR_DATA *victim,OBJ_DATA *arrow , int door, int chance ,int bonus) 
{
  ...
  if (ch->in_room != victim->in_room) {
	...
	if (IS_SET(pExit->exit_info, EX_BLOCKED)) {
	act("The exit is blocked.",ch,NULL,NULL,TO_CHAR);
	act("The exit is blocked.",ch,NULL,NULL,TO_ROOM);
	obj_to_room(arrow,ch->in_room);
	return;
	}
  }
	   ...
	   if (is_safe(ch,victim) || IS_SET(victim->imm_flags,IMM_ARROW)) {
		...
		if(IS_NPC(victim) && (number_percent() > 0 ) ) {
		  ...
		  if( ch->in_room == victim->in_room ) {
		    ...
		    act("$N {Yscreams and attacks you!!!{x",ch,NULL,victim,TO_CHAR);
		    multi_hit( victim, ch, TYPE_UNDEFINED );
		  }
		}
		else  { 
...
void do_shoot( CHAR_DATA *ch, char *argument )
{
  ...
  */
  ...
  if (!IS_NPC(victim) && victim->class == CLASS_CHANNELER) {
	 learned_dome = victim->pcdata->learned[skill_lookup("air dome")];
	 
	 if (learned_dome > 1 && IS_CHANNELING(victim)) {
	  // if ((number_percent() < learned) && (number_percent() < 75 )) {
		// If know gust of wind, then chance to grab dirst and throw back
		if (learned_dome >= 200) {
		  act("A dome of air in front of $n prevents an arrow from hitting $s!", victim,NULL,NULL,TO_ROOM);
		  act("$n tries to shoot an arrow at you, but you react and weave a {Cdome of air{x to protect from the arrow.", ch,NULL,victim,TO_VICT);
		  return;
		}
	  }
