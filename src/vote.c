...
bool is_tguild_poll_to (CHAR_DATA *ch, POLL_DATA *poll)
{
  ...
  /* My guild ? */
  if (is_full_name (player_tguild(ch), poll->to))
    return TRUE;
  ...
  if (can_tguild(ch) && (is_full_name("tguildleader", poll->to) ||
                          is_full_name("tguildleaders", poll->to)||
                          is_full_name(tguild_leader, poll->creator)))
    return TRUE;
...
bool is_poll_to (CHAR_DATA *ch, POLL_DATA * poll)
{
  ...
  /* SSGuild notes */
  if ((is_tguild(ch) && is_full_name (player_tguild(ch), poll->to)))
    return TRUE;
  
  /* SSGuild leaders */
  if (is_tguild(ch) && can_tguild(ch)) {
    sprintf(tguild_leader, "%sLeader", player_tguild(ch));
    if (is_full_name("tguildleader", poll->to)  || 
	   is_full_name("tguildleaders", poll->to) ||
	   is_full_name(tguild_leader, poll->creator)) {
	 return TRUE;
    }
