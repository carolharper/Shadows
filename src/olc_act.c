...
#define TGEDIT( fun )        bool fun( CHAR_DATA *ch, char *argument )
...
const struct olc_help_type help_table[] =
{
    ...
    {     "tguild",      tguild_flags,  "TGuild flags." },
...
const struct wear_type wear_table[] =
{
	...
	{	WEAR_WT_ONLY, 	ITEM_WEAR_WT_ONLY},
	{	WEAR_BT_ONLY,	ITEM_WEAR_BT_ONLY	},
	{	WEAR_BG_ONLY,	ITEM_WEAR_BG_ONLY	},
	{	WEAR_COTL_ONLY,	ITEM_WEAR_COTL_ONLY		},
    {	NO_FLAG,	NO_FLAG			}
};
...
/* It somehow fixed a bug in showing scroll/pill/potions too ?! */
	...
	case ITEM_QUIVER:  /* Code taken from Gary McNickle's WoTMUD code    */
			sprintf(buf,
			"[v0] # of Arrows   : [%d]\n\r"
			"[v1] # of Dice     : [%d]\n\r"
			"[v2] Type of Dice  : [%d]\n\r",
...
/* OLC		    do_help( ch, "liquids" );    */
	...
	case ITEM_QUIVER:     /* code taken from Gary McNickle's WoTMUD code */
		...
		case 3: 
            send_to_char( "SPECIAL WEAPON TYPE TOGGLED.\n\r\n\r", ch );
			pObj->value[3] ^= (flag_value( weapon_type2, argument ) != NO_FLAG
				? flag_value( weapon_type2, argument ) : 0 );
		    break;
		}
            break;
...
TGEDIT( tguildedit_flags )
{
  ...
  if ( argument[0] != '\0' )
  {
    EDIT_TGUILD( ch, pSguild );

    if ( ( value = flag_value( tguild_flags, argument ) ) != NO_FLAG )
    {
      pSguild->flags ^= value;
      send_to_char( "TGuild flag(s) toggled.\n\r", ch);
      return TRUE;
    }
  }
  
  send_to_char( "Syntax: flag [flag ID]\n\r"
                "Type '? tguild' for a list of valid flags.\n\r", ch );
  return FALSE;
}
...
TGEDIT( tguildedit_create )
{
  int i = 0;
  
  for (i=1; i < MAX_CLAN; i++) /* just loop through and find an open slot */
  {
    if (tguild_lookup(tguild_table[i].name) == 0)
    break;
  }
  ...
  if (i <= MAX_CLAN) /* open slot */
  {
    ...
    tguild_table[i].name = str_dup("New TGuild");
    tguild_table[i].who_name = str_dup("New TGguild");

    for (x = 0; x < MAX_RANK; x++)
    {
      tguild_table[i].rank[x].rankname = str_dup("Empty");
      tguild_table[i].rank[x].skillname = str_dup("");
    }

    pSguild = &tguild_table[i]; /* return new clan data */
    ch->desc->pEdit = pSguild;
    send_to_char("TGuild created.\n\r", ch);
    return TRUE;
  }  

  send_to_char("No room to create a new TGuild. Increase MAX_CLAN\n\r", ch);  
  return FALSE;
}
...
TGEDIT( tguildedit_list )
{
  ...
  sprintf(buf, "Num  TGuild Name      Flags"
		"\n\r-----------------------------------------------\n\r");
  add_buf(buffer, buf);
  ...
  for (i=1; i <= MAX_CLAN; i++)
  {
    if (tguild_table[i].name != NULL && tguild_table[i].name[0] != '\0') {
	 sprintf(buf,"[%2d]  %-16s Flags: [%s]\n\r", 
		    i, 
		    tguild_table[i].name,
		    flag_string( tguild_flags, tguild_table[i].flags ) );
      add_buf(buffer, buf);
    }
  }
...
TGEDIT( tguildedit_show )
{
  ...
  sprintf(buf, "Name     : %s %s %s\n\rWho Name : %-10s\n\r",
		pSguild->name, 
		IS_SET(pSguild->flags, TGUILD_CHANGED) ? "{D[{W*{D]{x" : "",
		IS_SET(pSguild->flags, TGUILD_DELETED) ? "{RMarked for Deletion!{x" : "",
		pSguild->who_name);
  add_buf(buffer, buf);
  ...
  sprintf(buf, "Flags:      [%s]\n\r\n\r", flag_string( tguild_flags, pSguild->flags ) );
  add_buf(buffer, buf);
...
TGEDIT( tguildedit_name )
{
  ...
  if (tguild_lookup(argument) != 0)
  {
    send_to_char("That tguild allready exists.\n\r", ch);
    return FALSE;
  }
