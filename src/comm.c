...
void bust_a_prompt( CHAR_DATA *ch )
{
  ...
  while( *str != '\0' ) {
    ...
    case 'r' :
	 ...
	 i = buf2; break;
    case 'R' :
	 if( /*IS_IMMORTAL( ch ) && */   // commenting out allows morts to see vnums
		ch->in_room != NULL )
  ...
  *point	= '\0';
  ...
  /* Flamevoid flag                                    */  
  /*--------------------------------------------------*/
   if (IS_AFFECTED(ch, AFF_FLAMEVOID)) {
	 buf2[0] = '\0';
	 sprintf(buf2, "({R%s{x)","fv");
	 send_to_char(buf2,ch);
	 }
...
//	    close_socket( d );
	...
	if ( check_reconnect( d, ch->name, TRUE ) )
	 ...
	 ch->pcdata->next_warderupdate = current_time;
	if (!IS_SET( ch->act, PLR_COLOUR ) )
        {
...
int colour( char type, CHAR_DATA *ch, char *string )
{
  ...
  case 'u':
    ...
    break;
  case 'U':
    if( col->tguild_talk[2] )
	 sprintf( code, "\e[%d;3%dm%c", col->tguild_talk[0], col->tguild_talk[1], '\a' );
    else
	 sprintf( code, "\e[%d;3%dm", col->tguild_talk[0], col->tguild_talk[1] );
    break;
  case 'v':
    if( col->race_talk[2] )
  ...
  for (vch = char_list; vch != NULL; vch = vch_next) {
    ...
    if (vch->sex != ch->sex) {
	 ...
	 if (vch->sex == SEX_MALE && ch->sex == SEX_FEMALE && vch->in_room == ch->in_room) {
	   send_to_char("Your skin prickles for a moment giving you goosebumps!\n\r", vch);
	 }	 
	 continue;
    }
...
void dice_spheres(CHAR_DATA *ch)
{
  ...
  for(i = 0; i < MAX_SPHERE; i++) {
    ...
    if (ch->main_sphere == i)
	 main_add = number_range(15, 35);

    sum = sphere + main_add;

    /* 5% chance to be stronger in this sphere */
    if (number_chance(5)) {
	 diceval = number_range(5, 20);
	 sum += diceval;
    }
    ...
    if (!str_cmp(ch->desc->host,"none.net") ||
	!str_cmp(ch->desc->host,"none.net"))
    {
	if (sum > 22)
	  sum += 0;
    }
