...
void do_sguildsave(CHAR_DATA *ch, char *argument)
{
  ...
  sprintf(buf, "%tguild.dat", DATA_DIR);
...
void do_tguildsave(CHAR_DATA *ch, char *argument)
{
  FILE *fp;
  int i;
  int value;
  ...
  for (i=1; tguild_lookup(tguild_table[i].name) != 0; i++)
  {
    if ( !IS_SET(tguild_table[i].flags, TGUILD_DELETED) && tguild_lookup(tguild_table[i].name) != 0)
    {
      fprintf(fp, "\nGuild\t%s~\n", tguild_table[i].name);
      ...
      fprintf(fp, "Rooms\t%d\t%d\t%d\n", 
                   tguild_table[i].room[0],  
                   tguild_table[i].room[1],
                   tguild_table[i].room[2]);

      for (value=0; value < MAX_RANK; value++)
      {  
        if (tguild_table[i].rank[value].rankname == NULL ||
            tguild_table[i].rank[value].rankname[0] == '\0')
           tguild_table[i].rank[value].rankname = str_dup("Unassigned");
            
          fprintf(fp, "Rank\t%d\t%s~\n", 
                       value+1, 
                       tguild_table[i].rank[value].rankname);
      }
        
      for (value=0; value < MAX_RANK; value++)
        if (tguild_table[i].rank[value].skillname != NULL &&
            tguild_table[i].rank[value].skillname[0] != '\0')
          fprintf(fp, "Skill\t%d\t%s~\n", 
                       value+1, 
                       tguild_table[i].rank[value].skillname);
      ...
      /* Remove "changed" bit before writing flags to file */
      if (IS_SET(tguild_table[i].flags, TGUILD_CHANGED))
        REMOVE_BIT(tguild_table[i].flags, TGUILD_CHANGED);
        
      fprintf( fp, "Flags %s \n",  fwrite_flag( tguild_table[i].flags,  buf ) );
  ...
  fprintf(fp, "\nEnd\n");
  fclose(fp);
  send_to_char("TGuild data file (tguild.dat) written.\n\r", ch);
  return;        
}
