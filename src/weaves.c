...
int	 find_hack_door	args( ( CHAR_DATA *ch, char *arg ) );   /*act_move.c */
//int find_door( CHAR_DATA *ch, char *arg )               /* act_move.c  */
...
{  
  ...
  if (!isAngreal) {
    ...
    sprintf(buf, "You combine flows of %s into a intricate pattern that you set on $p.", (char *)flow_text(sn, ch));
    act(buf, ch, angreal, NULL, TO_CHAR);
    ...
    ch->pcdata->next_createangreal = current_time+(60*60*2); /* Every other hour *///(86400); /* Every day*/
    // Log it
    ...
    for (pWeave = ch->in_room->weaves; pWeave != NULL; pWeave = pWeave_next) {
	 ...
	 if (str_cmp(cast_arg2, skill_table[pWeave->type].name))
	   ...
	   if ( (ch_skill > vic_skill) || (ch_flows > vic_flows) ) {
		...
		sprintf(buf, "You try to slice through %s's flow of %s, but fail!\n\r", PERS(victim_owner, ch), (char *) flow_text(pWeave->type, ch));
		send_to_char(buf, ch);
...
*/
  ...
  act("You suddenly feel as if {Cair{x entwines around you and prevents you from moving!", ch, NULL, victim, TO_VICT);
  act("You wrap $N with thin laces of {Cair{x, preventing them from moving.", ch, NULL, victim, TO_CHAR);
...
*       Author        : Zandor
*       Description   : Block someones windpipe, making it so they can't breathe
*       Parameters    : 
*       Returns       : 
**********************************************************************/
void spell_suffocate( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  act("You suddenly feel as if {Csomething{x prevents you from breathing!", ch, NULL, victim, TO_VICT);
  act("You smother $N with thin laces of {Cair{x, preventing them from breathing.", ch, NULL, victim, TO_CHAR);
...
void spell_shimmer( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  if ((victim = get_char_world(ch, target_name)) != NULL) {
    ...
    if (victim == ch) {
	 send_to_char("You can't shimmer to yourself.\n\r", ch);
	 return;
    }
    ...
    if (!IS_AFFECTED(ch, AFF_INVISIBLE))
       act("The air shimmers for a moment as it bends $n into existance.", ch, NULL, NULL, TO_ROOM);
...
void spell_heal_still( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  send_to_char("You return their ability to channel.\n\r",ch);
...
void do_token(CHAR_DATA *ch, char *argument)
{
  ...
  send_to_char("You can't feel the presence of this token right now.\n\rPerhaps it is lost?\n\r", ch);
  return;
}
...
void spell_minor_heal( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  if (ch->sex == SEX_FEMALE)
    ...
    send_to_char( "A burning feeling shoots through your body.\n\r", victim );
...
void spell_pinch( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  act("You pinch $N with a flow of {Cair{x.", ch, NULL, victim, TO_CHAR);
  if (victim->class == CLASS_CHANNELER) {
    if (victim->sex == ch->sex) {
	 act("$n pinches you with a flow of {Cair{x.", ch, NULL, victim, TO_VICT);
    }
    else {
...
void spell_gust_of_wind( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  if (portal == NULL && (door = find_exit( ch, cast_arg3 )) >= 0 ) {
    if ( (pexit = ch->in_room->exit[door]) != NULL ) {
	 if ( IS_SET(pexit->exit_info, EX_ISDOOR) )  {
	   ...
	   send_to_char("That direction is blocked by a wall of fire!\n\r", ch);
	   return;	
	 }
	 else if (IS_SET(pexit->exit_info, EX_AIRWALL)) {
	   send_to_char("That direction is blocked by a wall of air!\n\r", ch);
	   return;	
	 }
         else if (IS_SET(pexit->exit_info, EX_BLOCKED)) {
	   send_to_char("That direction is blocked by someone!\n\r", ch);
	   return;	
	 }	 
    }   
    else {
  ...
  if (portal == NULL) {
    sprintf(buf,"You call forth a gust of wind which tosses $N %sward.",dir_name[door]);
    act(buf,ch,NULL,victim,TO_CHAR);
    sprintf(buf,"A howl of wind picks you up and tosses you %sward.", dir_name[door]);
    act(buf,ch,NULL,victim,TO_VICT);
    sprintf(buf,"A howl of wind picks up $N and tosses $M %sward.", dir_name[door]);
    ...
    sprintf(buf, "A howl of wind roars in from the %s and $n is tossed into the room and to the ground.", dir_name[rev_dir[door]]);
    act( buf, victim, NULL, NULL, TO_ROOM );
  ...
  // portal gust
    ...
    if (IS_SET(portal->value[1],EX_CLOSED)) {
	 act("A howl of wind roars into the room and tosses $N toward the closed $p.",ch,portal,victim,TO_CHAR);
	 act("A howl of wind roars in and tosses you toward the closed $p.",ch,portal,victim,TO_VICT);
	 act("A howl of wind roars in, picking up $N and tossing $M toward the closed $p.",ch,portal,victim,TO_NOTVICT);
	 return;
    }
    ...
    if (location == NULL
	 ...
	 if (IS_SET(portal->value[2],GATE_DREAMGATE)) {
	   if (IS_DREAMING(victim)) {
		act("A howl of wind roars into the room and tosses $N toward the $p, but $E bounces back by an unseen force.",ch,portal,victim,TO_CHAR);
		act("A howl of wind roars into the room and tosses you toward the $p, but you bounce back by an unseen force.",ch,portal,victim,TO_VICT);
		act("A howl of wind roars into the room, tossing $N toward the $p, but $E bounces back by an unseen force.",ch,portal,victim,TO_NOTVICT);                	             	
		return;
	   }
	   
	   // gust into dreamgate
	   act("A howl of wind roars into the room and tosses $N into $p.",ch,portal,victim,TO_CHAR);
	   act("A howl of wind roars into the room and tosses you into $p.",ch,portal,victim,TO_VICT);
	   act("A howl of wind roars into the room, tossing $N into $p.",ch,portal,victim,TO_NOTVICT);                
	   TOGGLE_BIT(victim->world, WORLD_NORMAL);
...
void spell_thunderclap( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  act("You reach into the sky bringing {Dc{Wl{wou{Wd{Ds{x that generates a deafening and startling {Wthun{Dder{Wclap{x!", ch, NULL, NULL, TO_CHAR);
...
void spell_create_rain( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  if (!IS_OUTSIDE(ch)) {
    send_to_char("You start to reach into the sky, but suddenly realize that you are not outside!\n\r", ch);
    return;
  }
...
void spell_fireball( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  if (!IS_NPC(ch)) {
    if (ch->main_sphere == SPHERE_FIRE) {
	 mainsphere = TRUE;
	 dam += ch->perm_sphere[SPHERE_FIRE]/1;
    }
  }
  ...
  for (i = 1; i <= fireballs; i++) {
    if (fireballs > 1 && number_chance(5*i)) {
         if (i > 20)
	 ...
	 damage( ch, victim, number_range(dam/1, dam), sn, DAM_FIRE ,TRUE);
    }
  }
...
void spell_inferno( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  if (!IS_NPC(ch)) {
    if (ch->main_sphere == SPHERE_FIRE) {
	 mainsphere = TRUE;
	 dam += number_range(ch->perm_sphere[SPHERE_FIRE]/2, ch->perm_sphere[SPHERE_FIRE]);
    }
  }
...
void spell_earthquake( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  if (!IS_NPC(ch)) {
    if (ch->main_sphere == SPHERE_EARTH) {
	 mainsphere = TRUE;
	 dam += ch->perm_sphere[SPHERE_EARTH]*1;
    }
  }
  ...
  act("Your weave of pure {yearth{x reaches into the ground, causing deadly spikes to erupt and impale your enemies!", ch, NULL, NULL, TO_CHAR);
  act("Spikes of stone and metal erupt from around $n!", ch, NULL, NULL, TO_ROOM);
  ...
  for ( vch = char_list; vch != NULL; vch = vch_next ) {
    ...
    if ( vch->in_room == ch->in_room ) {
	 for (i = 1; i <= spikes; i++) {
	   if (vch != NULL) {
	      if (spikes > 1 && number_chance(4*i)) {
		     sprintf(buf, "Some deadly {yspikes{x miss you!", i > 20 ? numberorder[20] : numberorder[i]);
		...
		damage( ch, vch, number_range(dam/3, dam), sn, DAM_PIERCE,TRUE);
	      }
	   }
	  }
    }
  ...
  for ( vch = char_list; vch != NULL; vch = vch_next ) {
    ...
    if ( vch->in_room->area == ch->in_room->area
	    &&   IS_OUTSIDE(vch)
	    &&   IS_AWAKE(vch) )
	 send_to_char( "The {yearth{x trembles violently beneath your feet.\n\r", vch );
  }
}
...
void spell_lightningbolt( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  if ((wield_weapon) || (wear_shield)) {
    ...
    sprintf(buf, "$n lifts one hand and %s deadly fork%s of {Ylightning{w arcs toward you.", bolts > 20 ? numbername[20] : numbername[bolts], bolts <= 1 ? "" : "s");
    act(buf, ch, NULL, victim, TO_VICT);
    ...
    sprintf(buf, "$n lifts both hands and %s deadly fork%s of {Ylightning{w arcs toward you.", bolts > 20 ? numbername[20] : numbername[bolts], bolts <= 1 ? "" : "s");
    act(buf, ch, NULL, victim, TO_VICT);
    ...
    for (i = 1; i <= bolts; i++) {
	 if (bolts > 1 && number_chance(10*i)) {
	   ...
	   act(buf, ch,NULL,victim,TO_CHAR);
	   act(buf, ch,NULL,victim,TO_NOTVICT);
	   sprintf(buf, "The %s deadly forks of {Ylightning{w miss you!", i > 20 ? numberorder[20] : numberorder[i]);
	   act(buf, ch, NULL,victim,TO_VICT);
	 }
	 else {
            if (victim->position == POS_DEAD)
...
void spell_call_lightning( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  sprintf(buf, "$n looks up to the sky and %s {Ylightning{x fork%s flashes down and strikes at $N.", bolts > 20 ? numbername[20] : numbername[bolts], bolts <= 1 ? "" : "s");
  act(buf, ch, NULL, victim, TO_NOTVICT);
  ...
  sprintf(buf, "$n looks up to the sky and %s {Ylightning{x fork%s flashes down towards you!", bolts > 20 ? numbername[20] : numbername[bolts], bolts <= 1 ? "" : "s");
  act(buf, ch, NULL, victim, TO_VICT);
  
  act("{WThe hair on the back of your neck stands on end for a moment...{Ythen...{x", ch, NULL, victim, TO_VICT);
  
  for (i = 1; i <= bolts; i++) {
    if (bolts > 1 && number_chance(5*i)) {
...
void spell_waterball( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  for (i = 1; i <= waterballs; i++) {
    if (waterballs > 1 && number_chance(5*i)) {
...
void spell_blizzard( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  for ( vch = char_list; vch != NULL; vch = vch_next ) {
    ...
    if ( vch->in_room->area == ch->in_room->area
	    &&   IS_OUTSIDE(vch)
	    &&   IS_AWAKE(vch) )
	 send_to_char( "Thunder rumbles in the sky, and a cold breeze shoots through the air.\n\r", vch );
  }
}
...
void do_studyresidues( CHAR_DATA *ch, char *argument )
{
  ...
  if (ch->in_room->residues != NULL) {
   for (rd = ch->in_room->residues; rd != NULL; rd = rd->next) {
	...
	//send_to_char(buf, ch);
	if (number_percent() > chance) {
	  sprintf(buf, "You examine the residues that were woven with flows of %s and learn to weave '%s'!\n\r", (char *)flow_text(rd->sn ,ch), skill_table[rd->sn].name);
	  send_to_char(buf, ch);
	  ...
	  sprintf(buf, "You examine the residues that were woven with flows of %s but can't figure it out.\n\r", (char *)flow_text(rd->sn ,ch));
	  send_to_char(buf, ch);
	  ch->endurance -= endurance;
	  WAIT_STATE(ch,skill_table[sn].beats);
	  return;
  ...
  send_to_char("There are no more residues here for you to study!\n\r", ch);
  return;
}
...
void spell_bond(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
  ...
  //wiznet
  sprintf(buf, "%s weaves flows of {YSpirit{x and bonds poor %s!", ch->name, (char *)flow_text(sn, ch), victim->name);
...
void do_unbond( CHAR_DATA *ch, char *argument )
{
  ...
  //wiznet
  sprintf(buf, "%s unbonds %s!", ch->name, victim->name);
  wiznet(buf,NULL,NULL,WIZ_CHANNELING,0,0); 
...
{ 
  ...
  send_to_char("List of disguise images you have created for yourself:\n\r", ch);
  for (pos = 0; pos < MAX_DISGUISE; pos++) {
...
void load_disguise(CHAR_DATA *ch, char *argument)
{
  ...
  if (ch->class == CLASS_CHANNELER)
  {
     send_to_char("You set a web of disguise on yourself.\n\r", ch);  
     act("The air ripples around $n for a slight moment.", ch, NULL, NULL, TO_ROOM);
  }
  else
  {
...
void spell_eavesdropward( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  if (!IS_SET(ch->world, WORLD_NORMAL)) {
    send_to_char("You start to combine the flows but seem to be unable to form the weave here.\n\r", ch);
    return;
  }
...
void spell_light_ward( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  send_to_char("You draw upon the elements of {Rfire{x, {Bwater{x and {air{x and create a revealing wind of {Yglowing{x dust.\n\r", ch);
...
/*
 * Mind Crush
 * The ultimate single target for spirit
 * steve squires, 1/7/2013
 */
...
void spell_mind_crush( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
  ...
  // Added Damage modifier
  dam += number_range((ch->perm_sphere[SPHERE_SPIRIT] + ch->perm_sphere[SPHERE_SPIRIT])/5, (ch->perm_sphere[SPHERE_SPIRIT] + ch->perm_sphere[SPHERE_SPIRIT])/3);

  if (IS_CODER(ch)) {
    sprintf(buf, "[ {YCoder{x ]: MAX DAM 'mind crush' = %d (%s)\n", dam, mainsphere ? "main sphere" : "not main sphere");
    send_to_char(buf, ch);
  }
...
/*
 * arrows of fire
 * The ultimate single target for fire
 * steve squires, 1/8/2013
 */

void spell_arrows_of_fire( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  char buf[MAX_INPUT_LENGTH];
  int dam = 0; // Max Dam
  ...
  // Added Damage modifier
  dam += number_range((ch->perm_sphere[SPHERE_FIRE] + ch->perm_sphere[SPHERE_FIRE])/5, (ch->perm_sphere[SPHERE_FIRE] + ch->perm_sphere[SPHERE_FIRE])/3);

  if (IS_CODER(ch)) {
    sprintf(buf, "[ {YCoder{x ]: MAX DAM 'arrows of fire' = %d (%s)\n", dam, mainsphere ? "main sphere" : "not main sphere");
    send_to_char(buf, ch);
  }
...
/*
 * tornado
 * The ultimate single target for air
 * steve squires, 1/8/2013
 */
void spell_tornado( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
  ...
  // Added Damage modifier
  dam += number_range((ch->perm_sphere[SPHERE_AIR] + ch->perm_sphere[SPHERE_AIR])/5, (ch->perm_sphere[SPHERE_AIR] + ch->perm_sphere[SPHERE_AIR])/3);

  if (IS_CODER(ch)) {
    sprintf(buf, "[ {YCoder{x ]: MAX DAM 'tornado' = %d (%s)\n", dam, mainsphere ? "main sphere" : "not main sphere");
    send_to_char(buf, ch);
  }
  ...
  for (i = 1; i <= rings; i++) {
    ...
    rdam = number_range(dam/2, dam);
    rdam = (rdam * 5) / 4;
    damage(ch, victim, rdam, sn, DAM_WIND, TRUE);
  }

  return;
}

void spell_earthen_grasp( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  char buf[MAX_INPUT_LENGTH];
  int dam = 0; // Max Dam
  ...
  // Added Damage modifier
  dam += number_range((ch->perm_sphere[SPHERE_EARTH] + ch->perm_sphere[SPHERE_EARTH])/5, (ch->perm_sphere[SPHERE_EARTH] + ch->perm_sphere[SPHERE_EARTH])/3);

  if (IS_CODER(ch)) {
    sprintf(buf, "[ {YCoder{x ]: MAX DAM 'earthen grasp' = %d (%s)\n", dam, mainsphere ? "main sphere" : "not main sphere");
    send_to_char(buf, ch);
  }
...
void spell_tidal_wave( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  char buf[MAX_INPUT_LENGTH];
  int dam = 0; // Max Dam
  ...
  // Added Damage modifier
  dam += number_range((ch->perm_sphere[SPHERE_WATER] + ch->perm_sphere[SPHERE_WATER])/5, (ch->perm_sphere[SPHERE_WATER] + ch->perm_sphere[SPHERE_WATER])/3);

  if (IS_CODER(ch)) {
    sprintf(buf, "[ {YCoder{x ]: MAX DAM 'tidal wave' = %d (%s)\n", dam, mainsphere ? "main sphere" : "not main sphere");
    send_to_char(buf, ch);
  }
  ...
  for (i = 1; i <= rings; i++) {
    ...
    rdam = number_range(dam/2, dam);
    rdam = (rdam * 5) / 4;
    damage(ch, victim, rdam, sn, DAM_DROWNING, TRUE);
  }
...
bool foxhead_immune(CHAR_DATA *ch, int gsn) {
	 ...
	 int immune_sn[] = {
		...
		skill_lookup("Slow"),
		skill_lookup("Still"),
		skill_lookup("Mind crush"),
		skill_lookup("Arrows of Fire"),
		skill_lookup("Earthen Grasp"),
		skill_lookup("Tidal Wave"),
		skill_lookup("Spinning Earthfire"),
		skill_lookup("Inferno"),
		skill_lookup("Tornado"),
...
/***********************************************************
*		Function		: spell_blast_door
*		Author			: Morrigan
*		Description		: A spell to blow doors open
*		Parameters		:
*		Returns			:
*************************************************************/
void spell_blast_door( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	if(!IS_SET(ch->world,WORLD_NORMAL))  /*making sure caster isn't in T'A'R */
	{
		send_to_char("Somehow you are unable to combine the flows in that way here.\n\r", ch);
		return;
	}
	...
	if (IS_NULLSTR(cast_arg2)) 
	{
		send_to_char("Syntax: weave 'blast door' <direction>\n\r", ch);
		return;
	}
	...
	if ((door = find_exit(ch, cast_arg2)) != -1)  
	{   
		...
		{
			send_to_char( "This door doesn't seem to be locked.\n\r", ch );
			return;
		}
		...
		{
			if(IS_SET(pexit->exit_info,EX_EASY))  /* easy doors open without problem */
			{
				modifier = 0;
			}
			if(IS_SET(pexit->exit_info,EX_HARD))   /* make it a little harder */
			{
				modifier = 30;
			}
			if(IS_SET(pexit->exit_info,EX_INFURIATING)
				|| IS_SET(pexit->exit_info,EX_PICKPROOF))   /* make it harder still  */
			{
				modifier = 75;
			}
						
			if(ch->perm_sphere[SPHERE_AIR] > 84)   /* make it easier if moderately high air sphere */
			{
				modifier = modifier-25;
			}
			if(ch->perm_sphere[SPHERE_AIR] > 99)   /* make it quite a bit easier if high air sphere */
			{
			...
			/* take a random % and if the caster's skill minus the modifier  */
			/* is higher than the %, remove the locked/closed bits and open  */
			if(number_percent( ) < (get_skill(ch,sn) - modifier))  
			{													   	
			REMOVE_BIT(pexit->exit_info, EX_LOCKED);
			REMOVE_BIT(pexit->exit_info, EX_CLOSED);
			act( "The door is blasted open.\n\r", ch, NULL, pexit->keyword, TO_ROOM);
			send_to_char( "Your weave has blown the door open.\n\r", ch );
			...
			&&   pexit_rev->u1.to_room == ch->in_room )
				...
				act( "The $d crashes open explosively.\n\r", rch, NULL, pexit_rev->keyword, TO_CHAR );
				}
				return;
			}
			send_to_char( "The door seems to be a bit more stout than you were prepared for.\n\r" ,ch );
			//check_improve(ch,spell_blast_door,TRUE,1);
		}	
