...
void do_smote(CHAR_DATA *ch, char *argument )
{
  ...
  for (vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room) {
    ...
    last[0] = '\0';
    //If we're in the middle of a quoted statement, don't parse, just quote directly

    int quotecount = 0;
    char * quote = strstr(temp,"\"");
    if (quote != NULL) {
	quotecount++;
	...
	while (quote != NULL) {
		quote++;
		quote = strstr(quote,"\"");
		if (quote != NULL) quotecount++;
	}
	//if we have an even number of quotation marks, we're outside of the quotes, otherwise, we're inside, so just send it as is.
	if (quotecount %2 != 0) {
	 send_to_char(emote_parse(vch,argument),vch);
	 send_to_char("\r\n",vch);
	 reward_rp (ch);
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
  ...
  if (is_tguild(victim)) {
    has_guildinfo = TRUE;
    sprintf( buf, "TGuild: %16s  Rank: %2d  Title: %s{x\r\n",
		   player_tguild(victim), 
		   victim->tguild_rank+1, 
		   victim->tguild_title ? victim->tguild_title : player_tguild_rank(victim));
    send_to_char(buf, ch);
  }
...
//	 affect_strip(vch,gsn_plague);
  ...
  affect_strip(victim,gsn_blademaster); //blinded by masterforms
  affect_strip(victim,gsn_dirt); //blinded by dirt kick
  affect_strip(victim,gsn_concussionblow); //blinded by concussion blow
...
void do_copyinboot (CHAR_DATA *ch, char * argument)
{
 ...
 void copyover_recover ()
 {
   ...
   for (;;)
   {
	 ...
	 d->character->pcdata->next_warderupdate = current_time;
...
*/
  ...
  if (!str_cmp(arg2, "webvote")) {
    ...
    if (value < 0 || value > 86400) {
	 send_to_char("webvote window time range is from 0 to 86400 seconds (3 hours).\r\n", ch);
	 return;
    }
  ...
  if (!str_cmp(arg2, "skill")) {
    ...
    if (victim->pcdata->learned[sn] + value < 0 ||
      victim->pcdata->learned[sn] + value > 301) {
      send_to_char("Legal mortal skill range is from 0 to 301.\r\n", ch);
      return;
    }
...
void do_whoguild(CHAR_DATA *ch, char *argument)
{
  ...
  for (i=0; i < cnt; i++) {
    ...
    sprintf(buf, "%s%*s  %s%-16s{x  %s%-16s{x  %s%-16s{x  %s%-16s{x  %s%-18s{x  %s%-16s{x\r\n",
		  ...
		  is_tguild(pcs[i])                   ? "{x" : "{R",
		  is_tguild(pcs[i]) ? player_tguild(pcs[i])                    : "n/a",
		  pcs[i]->minion != 0                  ? "{x" : "{R",
