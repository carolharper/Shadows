...
*/
  ...
  if (!IS_NPC(ch) && ch->pcdata->learned[sn] < 1) {
    send_to_char("You don't know how to channel the One Power.\n\r",ch);
    return;
  }

  /* If room has no channeling flag */
  if (IS_SET(ch->in_room->room_flags,ROOM_STEDDING)) {
    send_to_char("It is as if the tranquilty and peace of this place prevent you from reaching the One Power.\n\r", ch);
    return;
  }
  if( is_wearing_foxhead(ch)) {
	 send_to_char( "You reach for the One POwer and feel something stopping you.\n\r", ch );
	 return;
    }
...
void spell_balefire(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
  ...
  if (IS_NPC(victim)) {
    ...
    return;
        ...
        /* FOR PC */
  if (!IS_IMMORTAL(ch) && (!IS_RP(ch) || !IS_RP(victim)))
  {
    send_to_char("You both must be IC for this to happen.",ch);
    return;
    }
    for ( iWear = 0; iWear < MAX_WEAR; iWear++ ) {
     if (( obj = get_eq_char(victim, iWear)) == NULL)
    ...
    if (IS_IMMORTAL(ch)) {
        if (!IS_NPC(victim)) {
        do_echo(ch,"A thread just got burned out of the pattern!\r\n{x");
...
*/
  ...
  if (argument[0] == '\0') {
    ...
    if (!str_cmp(arg, "all")) {
	 ...
	 if (!found) {
	   send_to_char("You sustain no weaves on yourself or others.\n\r", ch);
	 }
	 else {
	   if (!IS_NPC(ch))
    ...
    /* tie spesific weave */
	 ...
	 send_to_char("You sustain no weave with that name on yourself.\n\r", ch);
	 return;
    }
  }
...
void do_untie (CHAR_DATA *ch,  char *argument)
{
  ...
  if (argument[0] == '\0') {
    ...
    if (!str_cmp(arg, "all")) {
	 for ( paf = ch->affected; paf != NULL; paf = paf_next ) {
	   ...
	   if (skill_table[paf->type].spell_fun == spell_null)
		...
		if ((channie_strength > paf->tied_strength + 20) && (paf->casterId != ch->id)) {
		  sprintf(buffer, "You examine the knot on the '%s' weave and attempt to untie it but the knot is too complex!\n\r",
				skill_table[paf->type].name);
		  send_to_char(buffer, ch);
		}
		else {
	 ...
	 if (!found) {
	   send_to_char("You find no weaves that are tied on you.\n\r", ch);
	 }
	 return;
    }
    ...
    /* tie spesific weave */
	 ...
	 for ( paf = ch->affected; paf != NULL; paf = paf->next ) {
	   if (str_prefix(arg, skill_table[paf->type].name)) {
		...
		channie_strength =  ch->holding + get_skill(ch, paf->type) + (IS_NPC(ch)) ? 0 : ch->pcdata->extended_level;
		if ((channie_strength > paf->tied_strength + 20) && (paf->casterId != ch->id)) {
		  sprintf(buffer, "You examine the knot on the '%s' and attempt to untie it but the knot is too complex!\n\r",
				skill_table[paf->type].name);
		  send_to_char(buffer, ch);
		  return;
		}
	 ...
	 send_to_char("You find no weave with that name that is tied on you.\n\r", ch);
	 return;
    }
  }
  ...
  /* WARDS */
    ...
    if (!str_cmp(arg, "all")) {
	 for (pWard = ch->in_room->wards; pWard != NULL; pWard = pWard_next) {
		...
		sprintf(buffer, "You examine the knot on the '%s' ward and attempt to untie it but the knot is too complex!\n\r",
			   skill_table[pWard->sn].name);
		send_to_char(buffer, ch);
	   }
	   else {
      ...
      if (!found) {
	   send_to_char("You find no wards that are tied here.\n\r", ch);
      }
      return;            
    }
  ...
  /* ROOM WEAVES */
    ...
    if (!str_cmp(arg, "all")) {
	 for (pWeave = ch->in_room->weaves; pWeave != NULL; pWeave = pWeave_next) {
		...
		sprintf(buffer, "You examine the knot on the '%s' room weave but the knot is too complex!\n\r",
      ...
      if (!found) {
	   send_to_char("You find no weaves that are tied in this room.\n\r", ch);
      }
      return;            
    }
    ...
    // Untie spesific ward
	 ...
	 for (pWeave = ch->in_room->weaves; pWeave != NULL; pWeave = pWeave_next) {
	   ...
	   if ((channie_strength > pWeave->tied_strength + 20) && (pWeave->casterId != ch->id)) {
		sprintf(buffer, "You examine the knot on the '%s' room weave and try to untie it but the knot is too complex!\n\r",
			   skill_table[pWeave->type].name);
		send_to_char(buffer, ch);
		return;
	   }
    ...
    if (!str_cmp(arg, "all")) {
	 for ( paf = vch->affected; paf != NULL; paf = paf_next ) {
	   ...
	   if (skill_table[paf->type].spell_fun == spell_null)
	    ...
	    channie_strength =  ch->holding + get_skill(ch, paf->type) + (IS_NPC(ch)) ? 0 : ch->pcdata->extended_level;
	    if ((channie_strength > paf->tied_strength + 20) && (paf->casterId != ch->id)) {
		sprintf(buffer, "You examine the knot on the '%s' weave and try to untie it but the knot is too complex!\n\r",
			   skill_table[paf->type].name);
		send_to_char(buffer, ch);
	   }
	   else {
	 ...
	 if (!found) {
	   sprintf(buffer,"You find no weaves that are tied on %s\n\r",  PERS(vch, ch));
	   send_to_char(buffer, ch);
	 }
	 return;
    }
...
void do_judge (CHAR_DATA *ch,  char *argument)
{
  ...
  if (vch_op < ch_op) {
    sprintf(buffer, "You are stronger overall in the Power than %s.\n\r", PERS(vch, ch));
    send_to_char(buffer, ch);
    ...
    sprintf(buffer, "%s seems to be stronger overall than you in the Power.\n\r", PERS(vch, ch));
    send_to_char(buffer, ch);
...
bool break_shield (CHAR_DATA *ch, char *argument)
{
  ...
  for ( paf = ch->affected; paf != NULL; paf = paf->next ) {
    ...
    if (paf->modifier < break_strength) {
	 send_to_char("You press against the shield and {Rbreak through{x!\n\r", ch);
...
void do_invert (CHAR_DATA *ch,  char *argument)
{
  ...
  if (argument[0] == '\0') {
    ...
    /* invert spesific weave */
	 ...
	 send_to_char("You find no weave with that name on yourself.\n\r", ch);
	 return;
    }
  }
...
void do_rremove( CHAR_DATA *ch, char *argument )
{
  ...
  if (learned < 1) {
    send_to_char("You don't know how to remove the vestiges of your flows.\n\r", ch);
    return;
  }
  ...
  for (rd = ch->in_room->residues; rd != NULL; rd = rd_next) {
    ...
    if (number_percent() > learned) {
	 sprintf(buf, "You examine the residues from the flows woven into '%s' but you fail to remove them.\n\r", skill_table[rd->sn].name);
    }
    else {
	 sprintf(buf, "You examine the residues from the flows woven into '%s' and remove them.\n\r", skill_table[rd->sn].name);
  ...
  send_to_char("You are unable to find any residues from any flows made by you here.\n\r", ch);
  return;            
}
...
void spell_align_matrix( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    OBJ_DATA *obj = (OBJ_DATA *) vo;
    AFFECT_DATA *paf; 
    int result, fail, chance;
    int ac_bonus, added;
	...
	if (obj->extra_flags >= ITEM_ALIGNED) {
		send_to_char("That item has already been aligned.\n\r",ch);
		return;
		}

	if  ((obj->item_type != ITEM_WEAPON) && (obj->item_type != ITEM_ARMOR)) {
		send_to_char("That item cannot be aligned.\n\r",ch);
		return;
		}

	if ((obj->item_type == ITEM_ARMOR || ITEM_WEAPON) && 
		 (obj->wear_loc != -1))
		{
		send_to_char("The item must be carried to be aligned.\n\r",ch);
		return;
		}
	...
	if (obj->item_type == ITEM_ARMOR) {	
		...
		ac_bonus = 0;
		fail = 0;	/* Talent weave so no base failure chance */
		...
		fail = number_range(0,fail);
		result = number_percent();
		chance = number_range(1, (get_skill(ch,spell_align_matrix)/20));
		...
		{
		act("You set the weave on $p, but are unable to align the matrices.\n\r",ch,obj,NULL,TO_CHAR);
			return;
			}
		...
		{
		act("You place the weave on $p and feel it snap in place!",ch,obj,NULL,TO_CHAR);
		act("$p seems to shiver for a moment.",ch,obj,NULL,TO_ROOM);
		SET_BIT(obj->extra_flags, ITEM_ALIGNED);
		added = -5;
		}
    
		else  /* exceptional enchant */
		{
		act("You weave a skein of masterfully placed threads around $p and feel it snap into place!",ch,obj,NULL,TO_CHAR);
		act("$p hums momentarily!",ch,obj,NULL,TO_ROOM);
		SET_BIT(obj->extra_flags,ITEM_ALIGNED);
		added = -10;
		}
		...
		obj->level = UMIN(LEVEL_HERO - 1,obj->level + 0);
	...
	if (obj->item_type == ITEM_WEAPON) {
		...
		hit_bonus = 0;
		dam_bonus = 0;
		fail = 0;	/* Talent weave so no base failure */
