...
void	show_list_to_char	   args( ( OBJ_DATA *list, CHAR_DATA *ch,
				    bool fShort, bool fShowNothing, bool fShowLevel, const int iDefaultAction ) );
...
*/
  ...
  case POS_DEAD:     strcat( buf, " is lying here in mortal agony!" ); break;
  case POS_MORTAL:   strcat( buf, " will die soon, if not aided." );   break;
...
*/
  ...
  if(IS_IMMORTAL(ch) && (!brief)) { 
    if ( victim != ch
	 ...
	 show_list_to_char( victim->carrying, ch, TRUE, TRUE, TRUE, TRUE );
    }
  }
...
void do_hood(CHAR_DATA *ch, char *argument)
{
  ...
  if (IS_WOLFSHAPE(ch)) {
     send_to_char("You need to wear a cloak to be able to hood yourself.\n\r", ch);
     return;
  }
  ...
  if ((obj = get_eq_char(ch, WEAR_ABOUT)) != NULL) {
    if(!IS_NULLSTR(obj->short_descr)) {
	 if ( (strstr(colorstrem(obj->short_descr), "cloak") == NULL) &&
	   ...
	   send_to_char("You need to wear a cloak to be able to raise your hood.\n\r", ch);
	   return;
	 }
    }
  }
  ...
  if (IS_NULLSTR(ch->pcdata->hood_appearance)) {
    send_to_char("You need to set your hood appearance first.\n\r", ch);
    return;
  }
  ...
  if (IS_SET(ch->app,APP_HOODED)) {
    ...
    send_to_char("You raise the hood of your cloak to conceal your face.\n\r", ch);
...
void do_veilappearance(CHAR_DATA *ch, char *argument )
{
  ...
  if (!IS_AIEL(ch) && !IS_TARABONER(ch)) {
    send_to_char("You are not one of the Aiel.\n\r", ch);
    return;  	
  }
...
void do_veil(CHAR_DATA *ch, char *argument)
{
  ...
  if (IS_WOLFSHAPE(ch)) {
     send_to_char("You need to wear a shofa to be able to raise your veil.\n\r", ch);
     return;
  }
  ...
  if ((obj = get_eq_char(ch, WEAR_HEAD)) != NULL) {
    if(!IS_NULLSTR(obj->short_descr)) {
	 if ((strstr(colorstrem(obj->short_descr), "shoufa") == NULL) && 
	   ...
	   send_to_char("You need to wear a shofa or veil to be able to raise your veil.\n\r", ch);
	   return;
	 }
    }
  }
  ...
  if (IS_NULLSTR(ch->pcdata->veil_appearance)) {
    send_to_char("You need to set a veil appearance first.\n\r", ch);
    return;
  }
  ...
  if (IS_SET(ch->app,APP_VEILED)) {
    ...
    sprintf(buf, "You raise %s around your face.\n\r", obj->short_descr);
    send_to_char(buf, ch);
...
void do_cloak(CHAR_DATA *ch, char *argument)
{
  ...
  if ((obj = get_eq_char(ch, WEAR_ABOUT)) != NULL) {
    if(!IS_NULLSTR(obj->short_descr)) {
	 if (strstr(colorstrem(obj->short_descr), "cloak") == NULL) {
	   send_to_char("You need to wear a cloak before you can close it.\n\r", ch);
	   return;
	 }
    }
  }
  ...
  if (IS_SET(ch->app,APP_CLOAKED)) {
    send_to_char("You open the cloak and reveal what was hidden beneath it.\n\r",ch);
...
void do_seeself(CHAR_DATA *ch, char *argument)
{
  ...
  if (IS_SET(ch->act,PLR_SEESELF)) {
    send_to_char("You will no longer see yourself in the room.\n\r",ch);
    REMOVE_BIT(ch->act,PLR_SEESELF);
  }
  else {
    send_to_char("You will now see yourself in the room.\n\r",ch);
...
//	   send_to_char("\n{RYou sense a ward have been set in place here!{x\n\r", ch);
   ...
   case ITEM_PORTAL:
	...
	case ITEM_QUIVER:
			...
			if (obj->value[0] > 1)
				sprintf(buffer,"The quiver holds %d arrows.\n\r",obj->value[0]);
	...
	case ITEM_CORPSE_PC:
		...
		break;
            ...
            if( IS_SET( obj->value[1], CONT_ENTERABLE ) && obj->who_in )
             show_list_to_char(obj->contains, ch, TRUE, FALSE, FALSE, TRUE);
            else
             show_list_to_char(obj->contains, ch, TRUE, TRUE, obj->carried_by == NULL ? TRUE : FALSE, TRUE );
            if( obj->who_in )
            {
...
void do_score( CHAR_DATA *ch, char *argument )
{
  ...
  if (ch->insanity_points >0)
  {
	...
	{
	   sprintf(buf,"Male Channeler Status: You feel a strange itch inside your head, and are sometimes nauseous. \n\r");
	}
	else
	if (ch->insanity_points > 10 && ch->insanity_points <= 20)
	{
	   sprintf(buf,"Male Channeler Status: Sometimes, you think thoughts that don't seem like they are yours.\n\r");
	}
	else
	if (ch->insanity_points > 20 && ch->insanity_points <= 30)
	{
	   sprintf(buf,"Male Channeler Status: Sometimes you hear voices speaking in a strange language in your head.\n\r");
	}
	else
	if (ch->insanity_points > 30 && ch->insanity_points <= 40)
	{
	   sprintf(buf,"Male Channeler Status: You understand the voice's language, now. Your voices say everyone else is insane.\n\r");
	}
	else
	if (ch->insanity_points > 40 && ch->insanity_points <= 50)
	{
	   sprintf(buf,"Male Channeler Status: Everyone wants to kill you! You better take care of them first!\n\r");
	}
	send_to_char(buf,ch);
  }
...
void do_who( CHAR_DATA *ch, char *argument )
{
   ...
   for (i=0; i < cnt; i++) {
     ...
     if ((pcs[i]->invis_level > ch->level) && (pcs[i] != ch))
        continue;
   ...
   if (argument[0] != '\0') {
      if (!str_cmp (argument, "name")) {
        ...
        return;
         ...
         show_list_to_char( ch->carrying, ch, TRUE, TRUE, FALSE, TRUE );
         return;
      }
   }
   else {
...
void do_peek( CHAR_DATA *ch, char *argument )
{
  ...
  if ( ( victim = get_char_room( ch, arg ) ) != NULL ) {
    ...
    if (check < chance) {
	 ...
	 check_improve(ch,gsn_peek,TRUE,4);
	 show_list_to_char( victim->carrying, ch, TRUE, TRUE, FALSE, TRUE );
...
void do_rp(CHAR_DATA * ch, char *argument)
{
  ...
  if (IS_RP(ch)) {
    ...
    ch->pcdata->rprewardtimer = current_time + (number_range (480, 600));
    char buff[256];
...
void do_sheathwherename(CHAR_DATA * ch, char *argument)
{
  ...
  if (strlen(sheat_name) > 20 || strlen(sheat_name) < 6) {
    send_to_char("Sheath where name must be between 6 and 20 characters long (no colors allowed).\n\r", ch);
    return;
  }
