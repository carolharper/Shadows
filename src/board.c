...
bool is_tguild_leader(CHAR_DATA * ch); /* tguild.c */
...
/* find out if a note is a TGuild note for THIS char */
bool is_tguild_note_to (CHAR_DATA *ch, NOTE_DATA *note)
{
  char tguild_leader[MAX_STRING_LENGTH];
  
  memset(tguild_leader, 0x00, sizeof(tguild_leader));
  ...
  /* My guild ? */
  if (is_full_name (player_tguild(ch), note->to_list))
    return TRUE;
  
  /* Guild leader? */
  if (can_tguild(ch)) 
    sprintf(tguild_leader, "%sLeader", player_tguild(ch));
  
  if (can_tguild(ch) && (is_full_name("tguildleader", note->to_list) ||
                          is_full_name("tguildleaders", note->to_list)||
                          is_full_name(tguild_leader, note->sender)))
    return TRUE;
...
bool is_note_to (CHAR_DATA *ch, NOTE_DATA *note)
{
  ...
  /* TGuild notes */
  if ((is_tguild(ch) && is_full_name (player_tguild(ch), note->to_list)))
    return TRUE;
  
  /* TGuild leaders */
  if (is_tguild(ch) && can_tguild(ch)) {
    sprintf(tguild_leader, "%sLeader", player_tguild(ch));
    if (is_full_name("tguildleader", note->to_list)  || 
	   is_full_name("tguildleaders", note->to_list) ||
	   is_full_name(tguild_leader, note->sender)) {
	 return TRUE;
    }
...
int unread_tguild_notes (CHAR_DATA *ch, BOARD_DATA *board)
{
  NOTE_DATA *note;
  time_t last_read;
  int count = 0;
  ...
  for (note = board->note_first; note; note = note->next)
    if (is_tguild_note_to(ch, note) && ((long)last_read < (long)note->date_stamp))
      count++;
	   ...
	   if (!quiet) {
		...
		if (is_note_to(victim, ch->pcdata->in_progress)) {
		  ...
		  /* SGuild */
		    ...
		    sprintf(buf, "{W[{CNote{W]:{x New {TGuild Note{x: %d  In Area: %s  From: %s.\n\r"
				  "        Subj: %s\n\r"
				  "        Text: {B%s...{x\n\r",
				  notenr+1,
				  boards[board_number (ch->pcdata->board)].short_name,
		  ...
		  /* TGuild */
		  else if (is_tguild_note_to(victim, ch->pcdata->in_progress)) {
		    sprintf(buf, "{W[{CNote{W]:{x New {UTGuild Note{x: %d  In Area: %s  From: %s.\n\r"
				  "        Subj: %s\n\r"
				  "        Text: {B%s...{x\n\r",
				  notenr+1,
				  boards[board_number (ch->pcdata->board)].short_name,
  ...
  if (!argument[0]) { /* show boards */
    ...
    if (use_christmas_layout()) {
	 send_to_char("{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{g-{R-{x\n\r"
			    "{x    {WName        Unread   Guild  SGuild TGuild   Pers.    Imm.   Total{x\n\r"
	 ...
	 send_to_char ("{W----------------------------------------------------------------------{x\n\r"
				"{x    {CName        Unread   Guild  SGuild TGuild   Pers.    Imm.   Total{x\n\r"
				"{W----------------------------------------------------------------------{x\n\r",ch);
    }
    for (i = 0; i < MAX_BOARD; i++) {
	 ...
	 tguild  = unread_tguild_notes(ch,&boards[i]);    /* how many tguild notes?    */
