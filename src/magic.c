...
// Skill in slice also count
  ...
  if (vch->holding > 0)
	{
	chance = (vch_flows + (vch->holding/10) - ch_flows);
		if (chance < 0)
			{
			chance = 10;
			}
...
bool saves_spell( int level, CHAR_DATA *victim, int dam_type )
{
    ...
    if (IS_NPC(victim))
    	save = 50  + ( victim->level - level) * 4 - victim->saving_throw * 3;
    else
	save = 50 + (victim->level + victim->pcdata->extended_level - level) * 4 - victim->saving_throw * 3;
...
void spell_armor( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    if (get_curr_flows(ch) >= MAX_FLOWS) {
	 send_to_char("Your strength prevents you from substaining any more flows.", ch);
	 return;
    }
...
void spell_cancellation( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    {
        act("$n looks relieved as the weight holding them down is removed.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }
...
void spell_harm( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
  ...
  act("$n raises $s arm slightly and a instant {rpain{x shoots through your head!.", ch, NULL, victim, TO_VICT);
  act("You aim a series of deadly flows of {Yspirit{x toward $N's head with {rHarm{x!.", ch, NULL, victim, TO_CHAR);
  act("$n raises $s arm slightly towards $N, and $E clutch $S head!", ch, NULL, victim, TO_NOTVICT);

  int minharms = 1;
  int maxharms = 6; //ch->pcdata->learned[sn] / 30; 
  if (!IS_NPC(ch))
  {
...
void spell_haste( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
	...
	if (!check_dispel(level,victim,skill_lookup("slow")))
	{
	    if (victim != ch)
	        send_to_char("Weave failed.\n\r",ch);
	    send_to_char("The weight holding you down is lifted.\n\r",victim);
	    return;
	}
        act("$n looks relieved as the weight holding them down is removed.",victim,NULL,NULL,TO_ROOM);
        return;
...
void spell_slow( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    {
	if (victim != ch)
            act("You attempt to weave strands of {yEarth{x to weigh $N down, but fail.\n\r",ch,NULL,victim,TO_CHAR);
    ...
    if (IS_AFFECTED(victim,AFF_HASTE))
    {
	    ...
	    if (victim != ch)
              act("You attempt to weave strands of {yEarth{x to weigh $N down, but fail.\n\r",ch,NULL,victim,TO_CHAR);
              act("You feel like a great weight suddenly rests on your shoulders, slowing you down.\n\r",victim,NULL,NULL,TO_VICT);
            return;
        }
    ...
    affect_to_char( victim, &af );
    send_to_char("You feel like a great weight suddenly rests on your shoulders, slowing you down.\n\r",victim);
    act("$N staggers momentarily.",ch,NULL,victim,TO_CHAR);
    return;
}
