...
*/
  ...
  victim->rank = cnt;
  if (cnt <= 2 && !IS_SET(victim->act, PLR_MORTAL_LEADER))
  {
    SET_BIT(victim->act, PLR_MORTAL_LEADER);
    send_to_char("They are now a mortal leader.\n\r", ch);
    send_to_char("You have just been promoted to a leader of your guild!\n\r", victim);
...
void do_tgrecall( CHAR_DATA *ch, char *argument )
{
  char buf[MAX_STRING_LENGTH];
  CHAR_DATA *victim=NULL;
  ROOM_INDEX_DATA *location=NULL;
  ...
  if (!IS_SET(ch->act, ACT_PET) && !is_tguild(ch)) {
    send_to_char("You aren't in an tguild.\n\r",ch);
    return;
  }
  ...
  if (!IS_NPC(ch)) {
    if ((location = get_room_index(tguild_table[ch->tguild].room[0] )) == NULL) {
	 send_to_char("Your guild haven't set up a recall yet.\n\r", ch);
	 return;
    }
  }  
