...
void reset_tguild_status(CHAR_DATA *ch); /* tguild.c    */
...
*/
  ...
  // Reset guild, sguild, tguild, df and minions    	
  reset_gstatus(ch);
  reset_oguild_status(ch);
  reset_sguild_status(ch);
  reset_tguild_status(ch);
  reset_dfstatus(ch);
  reset_mstatus(ch);
  ...
  if (IS_SET(ch->merits, MERIT_AESSEDAI))
	REMOVE_BIT(ch->merits, MERIT_AESSEDAI);
}
...
int get_obj_number( OBJ_DATA *obj )
{
	...
	||  obj->item_type == ITEM_KEY || obj->item_type == ITEM_TOKEN
	||  obj->item_type == ITEM_ORE || obj->item_type == ITEM_GEMSTONE
...
//       	log_string("## can_see 6");   	
   ...
   if (room_is_dark( ch->in_room ) && !IS_AFFECTED(ch, AFF_INFRARED) && (ch->race != race_lookup("fade"))) {
...
char *affect_bit_name( int vector )
{
	...
	if ( vector & AFF_FLAMEVOID     ) strcat(buf, " flamevoid"      );
...
char *extra_bit_name( int extra_flags )
{
    ...
    buf[0] = '\0';
    if ( extra_flags & ITEM_ALIGNED      ) strcat( buf, " aligned"    );
	...
	if ( extra_flags & ITEM_GUILD		 ) strcat( buf, " guild"      );
...
char *wear_bit_name(int wear_flags)
{
	...
	if (wear_flags & ITEM_WEAR_WT_ONLY) strcat(buf, " whitetoweronly");
...
int get_points( int race, int class )
{
 ...
 void default_colour( CHAR_DATA *ch )
 {
	...
	ch->pcdata->tguild_talk[0] = ( BRIGHT );
 ...
 void all_colour( CHAR_DATA *ch, char *argument )
 {
	...
	ch->pcdata->tguild_talk[1] = colour;
