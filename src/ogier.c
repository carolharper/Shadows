...
void do_singwood ( CHAR_DATA *ch, char *argument )
{
  ...
  if (!obj_created) {
    ...
    act("$n sits down next to $p and sings to it.", ch, tree_obj, NULL, TO_ROOM);
    return;
  }
  else {
    // Fix the strings
    ...
    sprintf(buf, "Slowly a part of %s reforms into $p until the two separates.", tree_obj->short_descr);
    act(buf, ch, obj, NULL, TO_CHAR);
    
    act("$n sits down next to $p and sings to it.", ch, tree_obj, NULL, TO_ROOM);
    sprintf(buf, "Slowly a part of %s reforms into $p until the two separate.", tree_obj->short_descr);
    act(buf, ch, obj, NULL, TO_ROOM);
	 
    if (CAN_WEAR(obj, ITEM_TAKE)) {
	 obj_to_char( obj, ch );
	 act("You gently pick up $p and lower your voice.", ch, obj, NULL, TO_CHAR);
	 act("$n gently picks up $p and lowers $s voice.", ch, obj, NULL, TO_ROOM);
    }
    else {
	 // Can pick up noget items, but once they are dropped they are no good to take again.
    	 act("Your huge size makes it possible to pick up $p before you lower your voice.", ch, obj, NULL, TO_CHAR);
	 act("$n is so huge that picking up $p seems easy!", ch, obj, NULL, TO_ROOM);
	 act("$n lowers $s voice.", ch, NULL, NULL, TO_ROOM);
