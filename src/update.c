...
void gain_burnout(CHAR_DATA *ch, int bos)
{
  ...
  if ((ch->burnout + bos) >= ch->max_burnout) {
    ...
    send_to_char("{rYou fall to the ground as you channel for the last time.{x\n\r", ch);
...
void handle_mc_insanity(CHAR_DATA *ch)
{
	...
	{
	   case 5:
         	  act("You grab your head as you feel a horrible headache coming on, your head feels like its being split open.", ch, NULL, NULL, TO_CHAR);
	   ...
	   case 10:
		  ...
		  break;
	   case 15:
         	  act("Your vision clouds for a moment, and you are gripped with sudden terror!", ch, NULL, NULL, TO_CHAR);
         	  act("$n suddenly looks terrified.", ch, NULL, NULL, TO_ROOM);
		  ...
		  break;
	   case 20:
         	  act("You jerk suddenly, grabbing your head and falling to the floor in pain.", ch, NULL, NULL, TO_CHAR);
         	  act("$n loses his balance and grabs his head, falling to the floor.", ch, NULL, NULL, TO_ROOM);
		 ...
		 do_function(ch,&do_cast,"'earthquake'");
		  ...
		  break;
  	   case 25:
         	  act("The pain is so intense you collapse to the ground with a scream of pain escaping from your mouth.", ch, NULL, NULL, TO_CHAR);
         	  act("$n lets out a scream of pain before falling unconcious to the ground.", ch, NULL, NULL, TO_ROOM);
	          do_function(ch,&do_seize,"");
		  raw_kill(ch);
		  break;
	   case 30:
         	  act("You grab your chest in pain unable to catch your breath.", ch, NULL, NULL, TO_CHAR);
         	  act("$n grabs his chest noticably in pain, unable to catch his breath.", ch, NULL, NULL, TO_ROOM);
		  do_function(ch,&do_nofollow,"");
		  do_function(ch,&do_nofollow,"");
	   ...
	   case 35:
		  ...
		  break;
	   case 40:
         	  act("You lose momentary control as the voices take over.", ch, NULL, NULL, TO_CHAR);
		  ...
		  unstoppableweave(ch,NULL,"inferno");
                  break;
	   case 45:
         	  act("You lose control, flames leaping up around you as you give in to the voices.", ch, NULL, NULL, TO_CHAR);
         	  act("$n lets out a scream as flames erupt from the ground.", ch, NULL, NULL, TO_ROOM);
		  do_function(ch,&do_nofollow,"");
		  do_function(ch,&do_nofollow,"");
		  unstoppableweave(ch,NULL,"earthquake");
		  unstoppableweave(ch,NULL,"inferno");
		  ...
		  break;
           case 48:
         	  act("You lose control, the ground begins to shake and flames begin to leap up everywhere.", ch, NULL, NULL, TO_CHAR);
...
void check_talent(CHAR_DATA *ch)
{
  ...
  int lost_talent_chance  = ch->pcdata->forcespark ? 0 : 99;
...
long stock_exp( CHAR_DATA *ch)
{
  ...
  int lvl_limit=5;        /* Levels to stock up exp */
  //int i=0;
...
void gain_exp( CHAR_DATA *ch, int gain )
{
  ...
  if (ch->exp > stock_exp(ch) && get_level(ch) < LEVEL_HERO - 1) {
...
*/
  ...
  if (get_level(ch) >= MAX_NORMAL_LEVEL)
  {
	...
	return;
     ...
     if (is_masterform(skillsn) || skillsn == gsn_trollocwarfare)
     {
  ...
  for ( ch = char_list; ch != NULL; ch = ch_next ) {
    ...
    if (ch->pIndexData->pShop != NULL) /* give him some gold */
	 if ((ch->gold * 100 + ch->silver) < ch->pIndexData->wealth) {
	   ...
	   ch->silver += ch->pIndexData->wealth * number_range(1,20)/5000;
	 }
    ...
    if(!IS_NPC(vch) && !IS_SET(vch->comm, COMM_NOHINT)) {
	 ...
	 case 18:
  	   if (vch->class == CLASS_CHANNELER) 
	      ...
	      if (!bm_found) {
  		   send_to_char("Use the '{Wnewbie{x' command to ask questions to the newbie helpers, immortals and other newbies.\n\r",vch); 
	      }
	   ...
	   if (char_knows_masterform(vch) && (vch->pcdata->next_bmtrain <= current_time)) {
		    for (bm = char_list; bm != NULL; bm = bm->next) {
		       ...
		       if (IS_SET(bm->train_flags, TRAIN_MASTERFORMS) && number_percent() <= 33) {
		          bm_found = TRUE;
		          sprintf(buf, "Assistance from a weapons expert can be found in %s today.\n\r", !IS_NULLSTR(bm->in_room->area->name) ? bm->in_room->area->name : "Randland");
		          send_to_char(buf, vch);
...
/*
	...
	*/
		   ...
		   send_to_char("Use the '{Wnewbie{x' command to ask questions to the newbie helpers, immortals and other newbies.\n\r",vch); 
	   break;
	 case  1: send_to_char("See '{Whelp who{x' for description of the characters in front of the player names.\n\r",vch); break;
	 case  2: send_to_char("Use the '{Wgraduate{x' command to leave newbie school. Make sure you don't leave before you are ready!\n\r",vch); break;
	 case  3: send_to_char("See '{Whelp gain{x' or '{Whelp train{x' for learning and training skills or weaves.\n\r",vch); break;
	 case  4: send_to_char("Use the '{Wlevel{x' command to spend experience and to advance a level.\n\r",vch); break;
	 case  5: send_to_char("See '{Whelp combat_prompt{x' for description of the messages you see each round of a fight.\n\r",vch); break;
	 case  6: send_to_char("The {WOP{x in channelers prompt and score is short for the One Power. It is your sphere sum.\n\r",vch); break;
	 case  7: send_to_char("It is not possible to train STATS. It is possible to increase STATS with quest tokens.\n\r",vch); break;
	 case  8: send_to_char("It is easier to level when you have friends with you.\n\r",vch); break;
	 ...
	 case 29: send_to_char("Hit points and Endurance can be trained up to certain levels.  Don't forget to do so early in your leveling.", vch);break;
