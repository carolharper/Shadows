...
void do_wkname ( CHAR_DATA *ch, char *argument )
{
  ...
  if (!IS_WOLFKIN(ch) && IS_SET(ch->talents, TALENT_WOLFKIN)) {
   send_to_char("You are too young to name your self among the wolves!\n\r", ch);
   return;
  }
  ...
  if ( argument[0] == '\0' ) {
    send_to_char("Please specify a wolfkin name for your self.\n\r", ch);
    return;
  }
...
void do_entice (  CHAR_DATA *ch, char *argument )
{
  ...
  if ( ch->master != NULL || ( ch->leader != NULL && ch->leader != ch )) {
     send_to_char( "You need to lead the group to be able to control it!\n\r", ch );
     return;
  }
...
{  
  ...
  if ( argument[0] == '\0' ) {
    send_to_char("Please specify a wolf appearance for your self.\n\r", ch);
    return;
  }

  if (!IS_IMMORTAL(ch) && !IS_NULLSTR(ch->pcdata->wolf_appearance)) {
    send_to_char("You already have set a wolf appearance for yourself.\n\r", ch);
