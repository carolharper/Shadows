...
"{G(Quest obj){x The Sceptre of Courage is lying here, waiting to be returned to its owner."},

{"quest crown", 	"the Crown of Wisdom",
"{G(Quest obj){x The Crown of Wisdom is lying here, waiting to be returned to its owner."},

{"quest gauntlet", 	"the Gauntlets of Strength",
"{G(Quest obj){x The Gauntlets of Strength are lying here, waiting to be returned to its owner."},
...
*/
  { "2000 Gold Pieces",     "2000 gold",	500, FALSE, 2000,&ch->gold},
  { "50 Trains",    	       "trains", 	300, FALSE, 50,    &ch->train},
  { "1 Token",   	       "token", 	1000, TRUE, 9920, 0},
/*
  ...
  { "Foxhead Medallion",       "medallion", 	50000, TRUE, 5876,    0},
  { "Beautiful Sword",         "sword", 	10000, TRUE, 5877,    0},
*/
  { "Death Certificate",       "certificate", 	20000, TRUE, 5880,    0},
  { "Ruby-Encrusted Dagger",   "ruby", 	10000, TRUE, 5878,    0},
  { "Black Sphere",	       "sphere", 	7000,  TRUE, 44,    0},
  { "Sword",                   "sword", 	7000,  TRUE, 40050, 0},
  { "Dagger",                  "dagger", 	7000,  TRUE, 40051, 0},
  { "Lance",                   "lance", 	7000,  TRUE, 40052, 0},
  { "Spear",	               "spear", 	7000,  TRUE, 40053, 0},
  { "Axe",                     "axe", 		7000,  TRUE, 40054, 0},
  { "Flail",                   "flail", 	7000,  TRUE, 40055, 0},
  { "Mace",	               "mace",  	7000,  TRUE, 40056, 0},
  { "Whip",                    "whip",   	7000,  TRUE, 40057, 0},
  { "Polearm",                 "polearm", 	7000,  TRUE, 40058, 0},
  { "Staff",                   "staff", 	7000,  TRUE, 40059, 0},
  { "Swordbreaker",            "swordbreaker", 	5000,  TRUE, 40048, 0},
  { "Black Leather Bracer",    "bracer", 	3000,  TRUE, 40047, 0},
  { "Black Leather Belt",      "belt", 		3000,  TRUE, 40046, 0},
  { "Black Silk Cloak",        "cloak", 	5000,  TRUE, 40045, 0},
  { "Black Mail Backplate",    "backplate", 	5000,  TRUE, 40044, 0},
  { "Black Kite Shield",       "shield", 	5000,  TRUE, 40043, 0},
  { "Scale Mail Sleeves",      "sleeves", 	4000,  TRUE, 40042, 0},
  ...
  { "Black Plumed Helmet",    "helmet", 	5000,  TRUE, 40038, 0},
  { "Pure Black Breastplate", "breastplate", 	6000,  TRUE, 40037, 0},
  { "Short Silver Chain",     "chain", 		3000,  TRUE, 40036, 0},
  { "Silver Onyx Ring",       "ring", 		3000,  TRUE, 40035, 0},
  ...
  { "Silver Stud Earring",    "stud",	 	3000,  TRUE, 40031, 0},
 ...
 if (!strcmp(arg1, "list")) {
   ...
   for (i = 0; reward_table[i].name != NULL; i++) 
   {
	if ((reward_table[i].object == TRUE) && (reward_table[i].value == obj->pIndexData->vnum)) 
		...
		found = TRUE;
		obj_from_char(obj);
                int qpreturning = reward_table[i].cost;
   ...
   if (IS_SET(ch->act,PLR_QUESTING)) {
	...
	if (ch->pcdata->questobj > 0 && ch->pcdata->countdown > 0) {
     ...
     if ((ch->pcdata->questmob == -1 || (ch->pcdata->questobj && obj_found)) && ch->pcdata->countdown > 0) {
	  ...
	  reward = number_range(800,1500);
	  pointreward = number_range(60,100);
	  ...
	  ch->pcdata->questmob = 0;
	  ch->pcdata->questobj = 0;
	  ch->pcdata->nextquest = number_range(4,6);
	  ch->silver += reward;
   ...
   if (IS_SET(ch->act,PLR_QUESTING)) {
	...
	ch->pcdata->questmob = 0;
	ch->pcdata->questobj = 0;
	ch->pcdata->nextquest = number_range(10,15);
     ...
     for (mcounter = 0; mcounter < 99999; mcounter ++) {
       ...
       if (vsearch != NULL) {
	    if (quest_level_diff(ch->level, vsearch->level) == TRUE
		   ...
		   && IS_SET(vsearch->area->area_flags, AREA_QUEST)
...
//  if ( ( room = find_location( ch, victim->name ) ) == NULL ) {
  ...
  if (number_percent() < 40)  {
    ...
    while ( (room == NULL)
		  ...
		  || (!IS_SET(room->area->area_flags, AREA_QUEST))
