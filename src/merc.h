...
typedef struct  tguild_type       TGUILD_DATA;
...
#define MAX_STRING_LENGTH	 6400
#define MAX_INPUT_LENGTH	       1624
#define PAGELEN			   22
...
/** SSguild flags
 */
#define TGUILD_DELETED		A
#define TGUILD_CHANGED		B
#define TGUILD_INDEPENDENT 	C /* a "loner" guild */
#define TGUILD_WOLF          D
#define TGUILD_IMMORTAL		E /* immortal only clan */
...
#define MAX_PC_HP   3500
#define MIN_PC_END  300
#define MAX_PC_END  4000
...
#define MERIT_AESSEDAI		 (N)
#define MERIT_YELLOWAJAH     (O)
#define MERIT_BLUEAJAH       (P)
#define MERIT_REDAJAH        (Q)
#define MERIT_GREENAJAH      (R)
#define MERIT_BROWNAJAH      (S)
#define MERIT_WHITEAJAH      (T)
#define MERIT_GREYAJAH       (U)
...
#define OBJ_VNUM_CRAFTING_WEAPON   40067
...
#define ITEM_ALIGNED		(ee)
#define ITEM_GUILD			(ff)
...
#define ITEM_WEAR_WT_ONLY		(Y)
#define ITEM_WEAR_BT_ONLY		(Z)
#define ITEM_WEAR_BG_ONLY		(aa)
#define ITEM_WEAR_COTL_ONLY		(bb)
...
#define ROOM_VNUM_RECALL_OSENREIN 24105
#define ROOM_VNUM_RECALL_WHITETOWER 14422
#define ROOM_VNUM_RECALL_WT 14422
...
#define WEAR_WT_ONLY			31
#define WEAR_AIEL_ONLY			32
#define WEAR_COTL_ONLY			33
#define WEAR_BG_ONLY			34
#define WEAR_BT_ONLY			35
...
#define PLR_AUTOTIE			 (gg)  /* channeler only */
...
struct	pc_data
{
  ...
  //boolean for building - buildwalk - when set, moving in a direction where there isn't an exit will create one and dig the new room
  bool speedwalkbuild;
};
...
#define EDIT_TGUILD(Ch, tguild)  ( tguild = Ch->desc->pEdit )
...
#define TGUILD_FILE    "tguilds.dat"
...
/* tguild.c */
struct  tguild_type tguild_table[MAX_CLAN];
char *  tguild_bit_name  args( ( int tguild_flags ) );
bool    is_tguild         args( (CHAR_DATA *ch) );
bool    is_same_tguild    args( (CHAR_DATA *ch, CHAR_DATA *victim) );
int 	tguild_lookup	args( (const char *name) );
void 	do_tguild	args( (CHAR_DATA *ch, char *argument ) );
void	do_tguild_promote	args( (CHAR_DATA *ch, char *argument) );
char 	*player_tguild_rank	args( (CHAR_DATA *ch) );
char	*player_tguild	args( (CHAR_DATA *ch) );
bool	can_tguild	args( (CHAR_DATA *ch) );
bool    can_detguild     args( (CHAR_DATA *ch) );
bool    can_tguild_promote     args( (CHAR_DATA *ch) );
bool    can_tguild_demote      args( (CHAR_DATA *ch) );
...
#define		    AREA_QUEST    256    /* Quests items/mobs and the WM can populate here */
