	...
	switch (d->editor)
	{
		...
		case ED_HELP:
     	...
     	case ED_SGUILD:
		  ...
		  break;
     	case ED_TGUILD:
		  tguildedit (d->character, d->incomm);
		  break;	  
             case ED_POLL:
...
const struct olc_cmd_type * get_cmd_table( int editor )
{
	switch(editor)
	{
		...
		case ED_TGUILD: return tguildedit_table;
	}
	return NULL;
}
...
/* TGuild edit command table */
const struct olc_cmd_type tguildedit_table[] =
{
    {   "commands",	show_cmd_commands   },
    {   "create",	tguildedit_create	},
    {   "flag",		tguildedit_flags	},
    {   "list",		tguildedit_list	},
    {   "ml",           tguildedit_ml        },
    {   "name",		tguildedit_name	},
    {   "whoname",	tguildedit_whoname	},
    {   "rank",		tguildedit_rank	},
    {   "rooms",        tguildedit_rooms     },
    {   "show",		tguildedit_show	},
    {   "skill",	tguildedit_skill	},
    {   "?",		show_help	},
    {   NULL,		0		}
};
/*****************************************************************************
...
TGUILD_DATA *get_tguild_data( int tguild )
{
  if ( tguild <= MAX_CLAN && 
       (tguild_table[tguild].name != NULL && tguild_table[tguild].name[0] != '\0' ))
    return &tguild_table[tguild];
  
  return &tguild_table[0]; /* null clan */
}
...
void tguildedit( CHAR_DATA *ch, char *argument )
{
  TGUILD_DATA *pSguild;
  char command[MIL];
  char arg[MIL];
  ...
  if ( command[0] == '\0' )
  {
    tguildedit_show( ch, argument );
    return;
  }
  
  /* Search Table and Dispatch Command. */
  for ( cmd = 0; tguildedit_table[cmd].name != NULL; cmd++ )
  {
    if ( !str_prefix( command, tguildedit_table[cmd].name ) )
    {
      if ( (*tguildedit_table[cmd].olc_fun) ( ch, argument ) )
      {
	SET_BIT( pSguild->flags, TGUILD_CHANGED );
	return;
      }
      else
	   return;
...
void do_tguildedit( CHAR_DATA *ch, char *argument )
{
  TGUILD_DATA *pSguild;
  int value;
  char arg[MAX_STRING_LENGTH];
  ...
  pSguild = &tguild_table[1]; /* set default */
  
  argument	= one_argument(argument,arg);
  
  if ( is_number( arg ) ) {
    value = atoi( arg );
    pSguild = get_tguild_data( value );
    if (pSguild->name[0] == '\0') {
	 send_to_char( "That sguild does not exist.\n\r", ch );
	 return;
    }
    ...
    if ( !str_cmp( arg, "create" ) ) {
	 if ( ch->pcdata->security < 9 )
	   {
		send_to_char( "TGuildEdit : Insuffecient security to create new tguilds.\n\r", ch );
		return;
	   }
	 
	 tguildedit_create( ch, "" );
	 ch->desc->editor = ED_TGUILD;
	 return;
    }
  
  if (!IS_IMMORTAL(ch)) {
    send_to_char("Insuffecient security to edit TGuilds.\n\r",ch);
    return;
  }
...
void    do_speedbuild (CHAR_DATA * ch, char *argument)
{
	char buffer[256];
	...
	if (IS_NPC(ch)) {
		send_to_char("Not while in an NPC body.\r\n",ch);
	}
	ch->pcdata->speedwalkbuild = !ch->pcdata->speedwalkbuild;

	sprintf(buffer, "Speedwalk is %s.\n\r",ch->pcdata->speedwalkbuild ? "ENABLED" : "DISABLED");
...
void    do_alist (CHAR_DATA * ch, char *argument)
{
  ...
  if (!str_cmp(argument, "info") || (argument[0] != '\0')) {
    sprintf(buf, "\n\rTotal areas on Shadows of the Wheel: {W%d{x\n\r", top_area);
    add_buf(output, buf);
