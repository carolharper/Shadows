...
*/
  ...
  if (sn > -1) {
    ...
    if (ch->pcdata->learned[sn] >= MAX_PC_TRAIN) {
	 act("$N tells you '{7My training has come to it's limit my friend.{x'",
		ch,NULL,trainer,TO_CHAR);
	 return;
    }
    ...
    if (is_masterform(sn)) {
	 if (ch->pcdata->next_bmtrain > current_time) {
	  ...
	  if (ch->class == CLASS_WARRIOR) {   /* Let warriors train twice */
	  tr_cost = 2;
	  }
    ...
    if (is_masterform(sn)) {
	 ch->pcdata->next_bmtrain = current_time+21600; /* Once per day */
	 if (IS_CODER(ch)) {
	 ...
	 if (is_masterform(sn) && ch->pcdata->learned[sn] > 90 && ch->pcdata->learned[sn] == trainer->train_level) {
	   ...
	   act("$N continues, '{7I would like you to have the $p to aid you in your tasks. It's made by my master, {rF{Riresong{x the powerful. May it serve you well.{x'", ch,sword,trainer,TO_CHAR);
	   if (sword != NULL) {
...
void do_gain(CHAR_DATA *ch, char *argument)
{
  ...
  if (sn > -1) {
	 ...
	 if (ch->sex == SEX_MALE) {
	   ...
	   act("$N shakes his head saying, '{7I can't teach a fish to fly.{x'", ch,NULL,trainer,TO_CHAR);
	   return;
	 }
     }
...
*/
	...
	if (ch->race == race_lookup("aiel") && is_masterform(sn) && sn == gsn_blademaster)
	{
		act("$N tells you '{7An Aiel touching a sword?  Have you no ji?",ch,NULL,trainer,TO_CHAR);
		return;
	}
...
*/
    ...
    if (is_masterform(sn)) {
	 ch->pcdata->next_bmtrain = current_time+21600; /* Four times per day */
	 if (IS_CODER(ch)) {
...
void do_spells(CHAR_DATA *ch, char *argument)
{
  ...
  if (ch->class != CLASS_CHANNELER) {
    send_to_char("You cannot channel the One Power.\n\r", ch);
    return;
  }
...
void do_teach( CHAR_DATA *ch, char *argument )
{
  ...
  if (ch == victim) {
    ...
    // Teach self disabled
    send_to_char("You can't teach your self, try seeking knowledge from else.\n\r", ch);
    return;
...
*/
  ...
  /* Set timer if Masterform */
  if (is_masterform(sn)) {
    victim->pcdata->next_bmtrain = current_time+21600; /* Four times per day */
...
{
    int cost = 5000; //5000 silcer or 50 gold
    char arg[MAX_STRING_LENGTH];
    int nType;       //ore type
    ...
    if ( !IS_SET(ch->in_room->room_flags, ROOM_MINING_GOLD) &&
         ...
         !IS_SET(ch->in_room->room_flags, ROOM_MINING_IRON) &&
    ...
    if (arg[0] == '\0') {  //No type of ore specified, pick one 
        send_to_char("Valid options are iron, silver, gold, or copper.\n\r",ch);
		return;
		
		//bool found = FALSE;
	//while (!found) 
	// {
		// int choice = number_range(1,5);
		// switch (choice) {
			    ...
			    // break;
                        // case MINING_ORE_IRON:
                            // if (IS_SET(ch->in_room->room_flags, ROOM_MINING_IRON)) //IRON
                            // {
                                // found = TRUE;
                                // nType = MINING_ORE_IRON;
                            // }
			    // break;
  	...
  	if (!str_prefix(arg,"silver")) 
	{
  	...
  	if (!str_prefix(arg,"iron")) 
	{
		if (!IS_SET(ch->in_room->room_flags, ROOM_MINING_IRON)) {
			send_to_char("There isn't any iron available here for mining.\n\r",ch);
			return;
		}
		else 
		{
	...
	{
		send_to_char("Invalid option.  The valid options are gold, iron, silver, or copper.\n\r",ch);
		return;
		}
    }
...
{
    int cost = 5000; //5000 silver or 50 gold
    char arg[MAX_STRING_LENGTH];
    int nType;       //ore type
    int nQuality;    //ore quality
    ...
    {
	send_to_char("You grub around in the dirt and break a nail but you don't know the first thing about finding gems.\n\r",ch);
	return;
    }
	...
	return;
     ...
     if (arg[0] == '\0') {  //No type of ore specified, pick one 
        send_to_char("Valid options are diamond, emerald, or ruby.\n\r",ch);
		return;
	// {
  	...
  	if (!str_prefix(arg,"emerald"))
	{
		...
		{
			send_to_char("Invalid option.  The valid options are diamond, emerald or ruby.\n\r",ch);
			return;
		}
...
void do_smelt(CHAR_DATA *ch, char *argument)
{
	...
	if (!str_prefix(ore, "iron")) {
		nType = MINING_ORE_IRON;
	}
	else
	{
		send_to_char("Invalid type of ore. Choices are copper, silver, gold, or iron.\n\r", ch);
		return;
	}
...
void do_weaponsmith(CHAR_DATA *ch, char *argument)
{
	...
	if (argument[0] == '\0') 
	{
		send_to_char("Syntax: weaponsmith <Axe|Dagger|Mace|Sword|Spear> <iron> <ore type>\n\r",ch);
		return;
	}
	argument = one_argument(argument,slot);
	if (slot[0] == '\0') 
	...
	if (str_prefix(slot,"sword") && str_prefix(slot,"dagger") && str_prefix(slot,"axe") && str_prefix(slot,"spear") && str_prefix(slot,"mace")) {
		send_to_char("Weaponsmithing options are:  sword, dagger, axe, spear, or mace.\n\r", ch);
		return;
	}

        if (!str_prefix(ore1, "iron")) {
                nOre1Type = MINING_ORE_IRON;
        }
        else
        {
                send_to_char("Invalid type of ore. First ore must be iron.\n\r", ch);
                return;
        }
        ...
        if (!str_prefix(ore2, "iron")) {
                nOre2Type = MINING_ORE_IRON;
        }
        else
        {
                send_to_char("Invalid type of ore. Choices are copper, silver, or iron.\n\r", ch);
                return;
        }
	...
	//At this point, we know what the character wants to make, and what the character wants to use to make it, now we have to find out if the character has what it takes to make it.
	int coppercount = 0, silvercount=0, ironcount=0; 
	int copperpcount = 0, silverpcount = 0, ironpcount = 0;
            ...
            if ( obj->wear_loc == WEAR_NONE && (can_see_obj( ch, obj ) ) )
            {
				if (obj->item_type == ITEM_ORE) 
				{
					if (IS_SET(obj->value[0], MINING_ORE_COPPER))
					{
						...
						if (IS_SET(obj->value[1], MINING_QUALITY_PERFECT)) 
						{
							coppercount++;
						}
					...
					if (IS_SET(obj->value[0], MINING_ORE_IRON))
					{
						if (IS_SET(obj->value[1], MINING_QUALITY_EXCELLENT)) 
						{
							ironcount++;
	...
	if ((nOre1Type = MINING_ORE_IRON) && (ironcount <= 0 ))
		{
		send_to_char("You do not have the iron needed to complete the item.\n\r", ch);
		return;
		}
	if ((nOre2Type == MINING_ORE_IRON) && (ironcount <= 1 ))
		{
		send_to_char("You do not have the second iron needed to complete the item.\n\r", ch);
		return;
		}
	else if ((nOre2Type == MINING_ORE_SILVER) && (silvercount <= 0 ))
		{
		send_to_char("You do not have the silver needed to complete the item.\n\r", ch);
		return;
		}
	else if ((nOre2Type == MINING_ORE_COPPER) && (coppercount <= 0 ))
		{
		send_to_char("You do not have the copper needed to complete the item.\n\r", ch);
		return;
		}
		
	if ((coppercount < 0)  || (silvercount < 0) || (ironcount < 0) || (ironpcount< 0) || (silverpcount < 0) || (copperpcount < 0))
	{
		send_to_char("You do not have all of the required ores for that.\n\r", ch);
		return;
	}
		...
		switch (nOre1Type) 
	{
		case MINING_ORE_IRON: ironcount--; break;
		default: break;
	}
	...
	switch (ore1Obj->value[0]) 
	{
		...
		{
			dammod += modifier;
			//send_to_char("BUGTEST:  first ore is iron.\n\r",ch); //testing purposes
		}
		break;
	}
	
	bonusmod = number_range(2,6);
	modifier = number_range(5,10);
	...
	{ 
		...
		{ 
			if (IS_SET(ore2Obj->value[1], MINING_QUALITY_PERFECT))
			{
				hpmod += (modifier * bonusmod);
				endurmod += (modifier * bonusmod);
			}	
				...
				if (number_range(1,2) == 1) 
				{
					...
					hpmod += (modifier * (bonusmod/2));
				}
				else 
				{
					endurmod += (modifier * (bonusmod/2));
				}	
			}
		
			//send_to_char("BUGTEST:  Second ore is Copper.\n\r",ch); //testing purposes
		}
		break;
		case MINING_ORE_SILVER: 
		{
		...
		case MINING_ORE_IRON:
		{
			if (IS_SET(ore2Obj->value[1], MINING_QUALITY_PERFECT))
			{
				dammod += (modifier * (bonusmod/2));
			}
			else
			{
		
			dammod += modifier;
		//	send_to_char("BUGTEST:  Second ore is Iron.\n\r",ch); //testing purposes
			}
		}
		break;
	 }
	...
	//make the new weapon
	OBJ_DATA * newItem = create_object(get_obj_index(OBJ_VNUM_CRAFTING_WEAPON), 0);
	
	if (!str_prefix(slot,"axe") 
		&& (IS_SET(ore2Obj->value[1], MINING_QUALITY_PERFECT)) 
		&& (IS_SET(ore1Obj->value[1], MINING_QUALITY_PERFECT)) ) 
	{
		newItem->value[0] =  WEAPON_AXE;
		newItem->value[3] =  DAM_SLASH;
		strcpy(type, "axe");
	}
	else
	if (!str_prefix(slot,"axe") 
		&& (ore1Obj->value[1] != ore2Obj->value[1]) )
	{
		newItem->value[0] =  WEAPON_AXE;
		newItem->value[3] =  DAM_SLASH;
		newItem->value[1] =  12;
	...
	if (!str_prefix(slot,"dagger") 
		&& (IS_SET(ore2Obj->value[1], MINING_QUALITY_PERFECT)) 
		&& (IS_SET(ore1Obj->value[1], MINING_QUALITY_PERFECT)) )
	{
		newItem->value[0] =  WEAPON_DAGGER;
		newItem->value[3] = DAM_PIERCE;
		strcpy(type,"dagger");
	}
	else
	if (!str_prefix(slot,"dagger") 
	...
	if (!str_prefix(slot,"mace") 
		&& (IS_SET(ore2Obj->value[1], MINING_QUALITY_PERFECT)) 
		&& (IS_SET(ore1Obj->value[1], MINING_QUALITY_PERFECT)) )
		{
		newItem->value[0] = WEAPON_MACE;
		newItem->value[3] =  DAM_BASH;
		strcpy(type,"mace");
		}
	else
	if (!str_prefix(slot,"mace") 
	...
	if (!str_prefix(slot,"sword") 
		&& (IS_SET(ore2Obj->value[1], MINING_QUALITY_PERFECT)) 
		&& (IS_SET(ore1Obj->value[1], MINING_QUALITY_PERFECT)) )
	{
		newItem->value[0] =  WEAPON_SWORD;
		newItem->value[3]  = DAM_SLASH;
		strcpy(type, "sword");
	}
	else
	...
	if (!str_prefix(slot,"spear") 
		&& (IS_SET(ore2Obj->value[1], MINING_QUALITY_PERFECT)) 
		&& (IS_SET(ore1Obj->value[1], MINING_QUALITY_PERFECT)) )
	{
		newItem->value[0] =  WEAPON_SPEAR;
		newItem->value[3] =  DAM_PIERCE;
		strcpy(type,"spear");
	}
	else
	...
	int nSlot = ITEM_WIELD;
	SET_BIT(nSlot,ITEM_TAKE);
	SET_BIT(newItem->wear_flags, nSlot);


	
	if (nOre1Type == MINING_ORE_IRON && nOre2Type == MINING_ORE_IRON && IS_SET(ore1Obj->value[1], MINING_QUALITY_PERFECT) && IS_SET(ore2Obj->value[1], MINING_QUALITY_PERFECT))
		{
		sprintf(buffer,"a {Cmastercrafted{x pure {Di{wro{Dn{x %s",type);
		}
	else if (nOre1Type == MINING_ORE_IRON && nOre2Type == MINING_ORE_IRON)
		{
		sprintf(buffer, "a pure {Di{wro{Dn{x %s",type);
		}
	else if (nOre1Type == MINING_ORE_IRON && nOre2Type == MINING_ORE_SILVER && IS_SET(ore1Obj->value[1], MINING_QUALITY_PERFECT) && IS_SET(ore2Obj->value[1], MINING_QUALITY_PERFECT))
		{
		sprintf(buffer,"a {Cmastercrafted{x {Di{wro{Dn{x %s with inlaid {Ws{Di{Wl{Dv{We{Dr{x vines", type);
		}
	else if (nOre1Type == MINING_ORE_IRON && nOre2Type == MINING_ORE_SILVER)
		{
		sprintf(buffer, "an {Di{wro{Dn{x %s with inlaid {Ws{Di{Wl{Dv{We{Dr{x vines", type);
		}
	else if (nOre1Type == MINING_ORE_IRON && nOre2Type == MINING_ORE_COPPER && IS_SET(ore1Obj->value[1], MINING_QUALITY_PERFECT) && IS_SET(ore2Obj->value[1], MINING_QUALITY_PERFECT))
		{
		sprintf(buffer,"a {Cmastercrafted{x {Di{wro{Dn{x %s with inlaid {rc{Rop{rp{Re{rr{x vines", type);
		}
	else if (nOre1Type == MINING_ORE_IRON && nOre2Type == MINING_ORE_COPPER)
		{
		sprintf(buffer,"an {Di{wro{Dn{x %s with inlaid {rc{Rop{rp{Re{rr{x vines", type);
		}
	...
	sprintf(buffer,"%s lies here.",newItem->short_descr);
	newItem->description  = str_dup(buffer);
	sprintf(buffer, "crafted weapon %s", slot);
	newItem->name =  str_dup(buffer);
	newItem->owner = str_dup(ch->name);
...
void do_ironwill(CHAR_DATA *ch, char *argument){
	...
	modifier = -1 * (skill / 3);
	if(skill < 1){
		send_to_char("You don't know how to steel your will.", ch);
		return;
	}
    ...
    affect_to_char(ch, &af);
    act( "$n steels themself mentally.", ch, NULL, NULL, TO_ROOM );
    send_to_char("You steel yourself mentally.\n\r", ch);
	check_improve(ch,gsn_ironwill,TRUE,1);
    return;
}
...
void do_concussionblow(CHAR_DATA *ch, char *argument){
	...
	chance = get_skill(ch, gsn_concussionblow)/3;
	if(number_percent() > chance){
		...
		duration = get_skill(ch, gsn_concussionblow)/40;
		modifier = -1 * (get_skill(ch, gsn_concussionblow)/10);
		level = ch->level;
		...
		check_improve(ch,gsn_concussionblow,TRUE,1);
	}
...
void do_flamevoid (CHAR_DATA *ch, char *argument){
	...
	check_improve(ch,gsn_flamevoid,TRUE,1);   /*check for improvement */
    return;
}
