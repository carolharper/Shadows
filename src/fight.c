...
bool	check_extra_dodge	args( ( CHAR_DATA *ch, CHAR_DATA *victim ) );
...
void multi_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
  ...
  */
  ...
  //sixth attack chance if seventh attack is above 150
  if (get_skill(ch,gsn_sixth_attack) > 150)
  {
     chance = get_skill(ch,gsn_sixth_attack)/4;
  ...
  // ------------------------------------
  chance = get_skill(ch, gsn_staff) / 3;
  if (number_percent() < chance) {
    OBJ_DATA *wield=NULL;    
    wield = get_eq_char( ch, WEAR_WIELD );
    if (wield != NULL && wield->item_type == ITEM_WEAPON && wield->value[0] == WEAPON_STAFF) {
	 if (number_percent() < 25) {
	   act("You {Btwirl{x $p around in a {Rseries of attacks towards{x $N.", ch, wield, victim, TO_CHAR);
	   ...
	   chance = number_range(1, (get_skill(ch, gsn_staff) / 15));
	   for (i=0; i<chance; i++) {
		one_hit(ch , victim , dt, TRUE, 0);
	   }
	 }
  ...
  // Using a dagger, you can get extra hit
  // ------------------------------------
  chance = get_skill(ch, gsn_dagger) / 4;
  if (number_percent() < chance) {
    OBJ_DATA *wield=NULL;    
    wield = get_eq_char( ch, WEAR_WIELD );
    if (wield != NULL && wield->item_type == ITEM_WEAPON && wield->value[0] == WEAPON_DAGGER) {
	 if (number_percent() < 25) {
	   act("You attack $N, bringing $p around in a {Rdeadly flurry{x of flashing blades!", ch, wield, victim, TO_CHAR);
	   act("$n attacks you, bringing $p around in a {Rdeadly flurry{x of flashing blades!", ch, wield, victim, TO_VICT);
	   act("$n attacks $N, bringing $p around in a {Rdeadly flurry{x of flashing blades!", ch, wield, victim, TO_NOTVICT);
	   chance = number_range(1, (get_skill(ch, gsn_dagger) / 16));
	   for (i=0; i<chance; i++) {
		one_hit(ch , victim , dt, TRUE, 0);
	   }
	 }
	 ...
	 check_improve(ch, gsn_dagger,TRUE,6);
	 if (ch->fighting != victim )
	   return;
    }
  }
  
      // Using a sword, you can get extra hit
  // ------------------------------------
  chance = get_skill(ch, gsn_sword) / 4;
  if (number_percent() < chance) {
    OBJ_DATA *wield=NULL;    
    wield = get_eq_char( ch, WEAR_WIELD );
    if (wield != NULL && wield->item_type == ITEM_WEAPON && wield->value[0] == WEAPON_SWORD) {
	 if (number_percent() < 25) {
	   act("You attack $N with a {Rm{rer{Dcil{res{Rs{x series of strikes with $p!", ch, wield, victim, TO_CHAR);
	   act("$n attacks you {Rm{rer{Dciles{rsl{Ry{x with a series of strikes with $p!", ch, wield, victim, TO_VICT);
	   act("$n attacks $N {Rm{rer{Dciles{rsl{Ry{x with a series of strikes with $p!", ch, wield, victim, TO_NOTVICT);
	   chance = number_range(1, (get_skill(ch, gsn_sword) /16));
	   for (i=0; i<chance; i++) {
		one_hit(ch , victim , dt, TRUE, 0);
	   }
	 }
	 ...
	 check_improve(ch, gsn_sword,TRUE,6);
	 if (ch->fighting != victim )
	   return;
    }
  }
  
      // Using a whip, you can get extra hit
  // ------------------------------------
  chance = get_skill(ch, gsn_whip) / 4;
  if (number_percent() < chance) {
    OBJ_DATA *wield=NULL;    
    wield = get_eq_char( ch, WEAR_WIELD );
    if (wield != NULL && wield->item_type == ITEM_WEAPON && wield->value[0] == WEAPON_WHIP) {
	 if (number_percent() < 25) {
	   act("You unfurl $p and attack $N with a series of quick, deadly strikes!", ch, wield, victim, TO_CHAR);
	   act("$n unfurls $p and attacks you with a series of quick, deadly strikes!", ch, wield, victim, TO_VICT);
	   act("$n unfurls $p and attacks $N with a series of quick, deadly strikes!", ch, wield, victim, TO_NOTVICT);
	   chance = number_range(1, (get_skill(ch, gsn_whip) /16));
	   for (i=0; i<chance; i++) {
		one_hit(ch , victim , dt, TRUE, 0);
	   }
	 }
	 ...
	 check_improve(ch, gsn_whip,TRUE,6);
	 if (ch->fighting != victim )
	   return;
    }
  }
    ...
    case (3) : //bite
	 if (number_percent() <= (tw_skill/4)) {
	   ...
	   if ((tw_skill_victim) && (number_percent () <= tw_skill_victim/10) ) {
		...
		act("{wYou let $M take a {Rbite{x of your fist!{x",victim,NULL,ch,TO_CHAR);
...
bool damage(CHAR_DATA *ch,CHAR_DATA *victim,int dam,int dt,int dam_type, bool show) 
{
  ...
  if ( victim->position == POS_DEAD )
   ...
   if (IS_NPC(victim) && IS_AFFECTED(victim,AFF_SAP))
   {
     ...
     */
    if ( dam > 50000 && dt >= TYPE_HIT) {    	
	 sprintf(buf, "Damage <%d> more than 50000 points caused by %s!", dam, IS_NPC(ch) ? ch->short_descr : ch->name);
	 log_string(buf);	
	 wiznet(buf,ch,NULL,WIZ_SECURE,0,0);
	 if (!IS_IMMORTAL(ch) && !IS_GHOLAM(ch))
	   dam = 50000;

	 // If damage over 5500, and victim is channeler - they have chance to lose con and channeling
    ...
    if ( dt >= TYPE_HIT && ch != victim && 
	 ...
	 if ((!IS_NPC(victim) && IS_SET(victim->auto_act, AUTO_DODGE))  || IS_NPC(victim))
	   ...
	   if ( victim->class == CLASS_THIEF && number_range(0,4) == 3) {
	      if ( check_extra_dodge( ch, victim ) )
			   return FALSE;
	...
	*/
    ...
    // Log real damage
    if ( dam > 50000 && dt >= TYPE_HIT) {    	
	 sprintf(buf, "Real damage <%d> more than 50000 points cause by %s!", dam, IS_NPC(ch) ? ch->short_descr : ch->name);
	 log_string(buf);	
	 wiznet(buf,ch,NULL,WIZ_SECURE,0,0);	 


	 // If real damage over 5500, and victim is channeler - they lose con and channeling
    ...
    if ( !IS_NPC(victim) && IS_TAVEREN(victim) && IS_RP(victim) && victim->hit < 10 && !IS_IMMORTAL(ch) && !IS_GHOLAM(victim)) {
         ...
         switch (luck) {
            ...
            case 1: // A tree fall over your victim
               act("{WSuddenly a {ytree{W falls and hits $N, {Wknocking $M out!{x", victim, NULL, ch, TO_CHAR    );
               act("{WSuddenly a {ytree{W falls and hits you, {Wknocking you out!{x", victim, NULL, ch, TO_VICT    );               
               act("{WSuddenly a {ytree{W falls and hits $N, {Wknocking $M out!{x", victim, NULL, ch, TO_NOTVICT );               
               victim->hit = 1;
               ch->hit = -100;               
               break;
...
bool is_safe_spell(CHAR_DATA *ch, CHAR_DATA *victim, bool area )
{
  ...
  if (IS_GHOLAM(victim))
  {
	send_to_char("Your weave has no affect on this person!!\r\n",ch);
        if (!IS_NPC(victim))
	 ...
	 if ((wielded != NULL && wielded->value[0] == WEAPON_DAGGER) &&
	   ...
	   if (get_eq_char( ch, WEAR_WIELD ) == NULL) {
		...
		if (number_percent() <= 15) {
		  ...
		  act( "$N {Gcrosses $S daggers{x in a risky manuever, parrying $n's attack and aiming a solid kick!", ch, NULL, victim, TO_NOTVICT);
		  do_kick(ch, "");
		}
		else {
		  act( "You {Gcross your daggers{x in a risky manuever, parrying $n's attack!", ch, NULL, victim, TO_VICT);
		  act( "$N {Gcrosses $S daggers{x in a risky manuever, parrying your attack!", ch, NULL, victim, TO_CHAR);
		  act( "$N {Gcrosses $S daggers{x in a risky manuever, parrying $n's attack!", ch, NULL, victim, TO_NOTVICT);
		}
...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim )
{
  ...
  endcost = get_skill(victim,gsn_parry)/75;
  ...
  victim->endurance -= (6 - endcost);

  if (IS_SET(victim->merits, MERIT_HUGESIZE))
    victim->endurance -= (8 - endcost);
...
bool check_shield_block( CHAR_DATA *ch, CHAR_DATA *victim )
{
  ...
  endcost = get_skill(victim,gsn_shield_block)/75;
...
bool check_dodge( CHAR_DATA *ch, CHAR_DATA *victim )
{
  ...
  endcost = get_skill(victim,gsn_dodge)/75;
 ...
 * Check for dodge.
 */
bool check_extra_dodge( CHAR_DATA *ch, CHAR_DATA *victim )
{
  int chance;
  int endcost;
  ...
  if (victim->race == race_lookup("fade")) {
    ...
    act( "Your extra agility from training allows you to dodge $n's attack.", ch, NULL, victim, TO_VICT    );
    act( "$N dodges your attack.", ch, NULL, victim, TO_CHAR    );
    act( "$N dodges $n's attack.", ch, NULL, victim, TO_NOTVICT );
  }
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim, int total_levels )
{
  ...
  case -9 :	base_exp =  25;		break;
  case -8 :	base_exp =  50;		break;
  case -7 :	base_exp =  75;		break;
  case -6 : 	base_exp =  100;		break;
  case -5 :	base_exp =  125;		break;
  case -4 :	base_exp =  150;		break;
  case -3 :	base_exp =  175;		break;
  case -2 :	base_exp =  200;		break;
  case -1 :	base_exp =  225;		break;
  case  0 :	base_exp =  250;	break;
  case  1 :	base_exp =  275;	break;
  case  2 :	base_exp = 300;		break;
  case  3 :	base_exp = 325;		break;
  case  4 :	base_exp = 350;		break;
  case  5 :	base_exp = 375;		break;
  case  6 :	base_exp = 400;		break;
  case  7 :	base_exp = 400;		break;
  case  8 :	base_exp = 400;		break;
  case  9 :	base_exp = 400;		break;
  case 10 :	base_exp = 400;		break;
  } 
 ...
 //CAP XP per kill at 400 before the double xp and reward multiplier bonuses kick in
  if (xp > 400)
  {
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim,int dam,int dt,bool immune, int hit_location , int dam_type)
{
  ...
  else if ( dam <=  10 ) { vs = "bruise";	vp = "bruises";	}
  else if ( dam <=  20 ) { vs = "scratch";	vp = "scratches";		}
  else if ( dam <=  30 ) { vs = "nick";	vp = "nicks";		}
  else if ( dam <=  40 ) { vs = "graze";	vp = "grazes";		}
  else if ( dam <=  50 ) { vs = "hit";	vp = "hits";		}
  else if ( dam <=  60 ) { vs = "injure";       vp = "injures";		}
  else if ( dam <=  70 ) { vs = "maul";	vp = "mauls";	}
  else if ( dam <=  80 ) { vs = "decimate";	vp = "decimates";	}
  else if ( dam <=  90 ) { vs = "devastate";	vp = "devastates";		}
  else if ( dam <= 100 ) { vs = "MUTILATE";	vp = "MUTILATES";	}
  else if ( dam <= 115 ) { vs = "DISEMBOWEL";	vp = "DISEMBOWELS";	}
  else if ( dam <= 130 ) { vs = "DISMEMBER";	vp = "DISMEMBERS";	}
  else if ( dam <= 145 ) { vs = "MASSACRE";	vp = "MASSACRES";	}
  else if ( dam <= 160 ) { vs = "MANGLE";	vp = "MANGLES";		}
  else if ( dam <= 175 ) { vs = "*** DEMOLISH ***"; vp = "*** DEMOLISHES ***";			}
  else if ( dam <= 190 ) { vs = "=== OBLITERATE  ==="; vp = "=== OBLITERATES ===";			}
  else if ( dam <= 205 ) { vs = "!!! MAKE LIFE DIFFICULT !!!{x for"; vp = "!!! MAKES LIFE DIFFICULT !!!{x for"; }
  else if ( dam <= 220 ) { vs = "!!! RIP THE LUNGS !!!"; vp = "!!! RIPS THE LUNGS !!!{x from";			}
  else if ( dam <= 255 ) { vs = "do UNSPEAKABLE things{x to"; vp = "does UNSPEAKABLE things{x to";		}
  else if ( dam <= 280 ) { vs = "drop AN ANVIL{x on"; vp = "drops AN ANVIL{x on";		}
  else if ( dam <= 300 ) { vs = "get MEDIEVAL{x on"; vp = "gets MEDIEVAL{x on";		}
  else if ( dam <= 400 ) { vs = "DESTROY{x"; vp = "DESTROYS{x";}
  else if ( dam <= 500 ) { vs = "ERADICATE{x"; vp = "ERADICATES{x";}
  else                   { vs = "ANNIHILATE{x"; vp = "ANNIHILATES{x";		}
...
void do_whirlwind(CHAR_DATA *ch, char *argument){
	...
	for ( vch = char_list; vch != NULL; vch = vch_next ) { // Go through the character list
		...
		if (IS_AFFECTED(vch,AFF_CAMOUFLAGE))
			continue;
		//if ((!IS_GHOLAM(vch)) && (!is_same_group(vch, ch)) && (IS_SAME_WORLD(vch,ch)) && (!IS_AFFECTED(vch, AFF_HIDE)) && (!IS_AFFECTED(vch, AFF_CAMOUFLAGE)))
	    ...
	    if ( vch->in_room == ch->in_room && ch->master != vch){ // Check to see if they're in the same room and not the master
	    	if(vch->fighting == ch || bAll || (bDescriptive && (strstr(arg,vch->short_descr)|| is_name(arg,vch->name)))){ // Get em!
	    		fighting_against++;
	    		act("{wYou perform a flurry of {Rd{re{Dad{rl{Ry{x blows against $M!{x",ch,NULL,vch,TO_CHAR);
	    		act("{w$n performs a flurry of {Rd{re{Dad{rl{Ry{x blows against you!{x",ch,NULL,vch,TO_VICT);
	    		//one_hit(ch, vch, gsn_whirlwind, FALSE, 0); //
	    		multi_hit(ch, vch, gsn_whirlwind); //
	    	} else {
	    		act("{w$n attacks $s enemies with a flurry of {Rd{re{Dad{rl{Ry{x blows!{x",ch,NULL,vch,TO_NOTVICT);
	    	}
	    }
	}
...
//    }
  ...
  if (IS_NPC(victim)) 
  {
  	...
  	modifier =  (200 - (ch->pcdata->learned[gsn_assassinate] / 2 )  - (bsmod( ch, victim) /4 )) / 3; 
  ...
  if (IS_IMMORTAL(victim)) {
    ...
    if (pernum < modifier ) {
	 ...
	 act("$N drops lifeless to the ground as $n sinks $p deep into $S back!", ch, obj, victim, TO_NOTVICT);
...
void do_surrender( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !IS_NPC( ch ) && IS_NPC( mob ) && ( !HAS_TRIGGER( mob, TRIG_SURR ) || !mp_percent_trigger( mob, ch, NULL, NULL, TRIG_SURR ) ) ) {
    ...
    if (mob->pcdata->learned[gsn_bind] > 0 && IS_SET(mob->auto_act, AUTO_BIND)) {
	 act ( "$N quickly binds you so you can't escape!", ch, NULL, mob, TO_CHAR );
	 act ( "$N quickly binds $n so $e can't escape!", ch, NULL, mob, TO_NOTVICT );
...
bool check_critical(CHAR_DATA *ch, CHAR_DATA *victim)
{
  ...
  if ( number_range(1,300) > (get_skill(ch,gsn_critical)/2) )  /*  was 0,100 > 10  */
    return FALSE;
...
void do_charge( CHAR_DATA *ch, char *argument )
{
  ...
  if (from_distance) {
    ...
    if (victim == NULL) {
	 ...
	 if (exit_flag == EX_FIREWALL) {
	   ...
	   act("You ride into a {rwall of fire{x and are thrown off the horse as the {Rfire{x surrounds you!", ch, NULL, NULL, TO_CHAR);
	 ...
	 if (exit_flag == EX_AIRWALL) {
	   ...
	   sprintf(buf, "$n arrives upon $s mount in a wild charge from the %s.", dir_name[rev_dir[dir]]);
	   act(buf,  ch, NULL, NULL, TO_ROOM );
  ...
  if (from_distance) {
    ...
    sprintf(buf, "$n arrive upon $s mount in a wild charge from the %s.", dir_name[rev_dir[dir]]);
    act(buf,  ch, NULL, NULL, TO_ROOM );
  ...
  if (number_percent() < chance) {
    act("Upon your mount you wildly charge at $N, thrusting $p toward $S body!", ch, obj, victim, TO_CHAR );
    act("Upon $s mount $n charges wildly at you, thrusting $s $p toward your body!", ch, obj, victim, TO_VICT );
    act("Upon $s mount $n wildly charges at $N, thrusting $p toward $S body!", ch, obj, victim, TO_NOTVICT );
    ...
    act("Upon your mount you wildly charge at $N, but $E dodges out of the way!", ch, NULL, victim, TO_CHAR );
    act("Upon $s mount $n wildly charges at you, but you quickly dodge out of $s way.", ch, NULL, victim, TO_VICT );
    act("Upon $s mount $n wildly charges at $N, but $E dodges out of the way!", ch, NULL, victim, TO_NOTVICT );
...
void do_overwhelm( CHAR_DATA *ch, char *argument )
{
  ...
  if (number_percent() < chance && number_percent() < 25 ) {
    ...
    act("$n tries to overwhelms you, but charges into the mount and is knocked to the ground!{x",ch,NULL,victim,TO_VICT);
    act("You try to overwhelm $N, but charge into the mount and is knocked to the ground!{x",ch,NULL,victim,TO_CHAR);
    act("$n tries to overwhelms $N, but charges into the mount and is knocked to the ground!{x",ch,NULL,victim,TO_NOTVICT);
