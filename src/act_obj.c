...
{
   return TRUE;
// idk wtf happened here, but it was always returnign true.
/*
...
void do_wear( CHAR_DATA *ch, char *argument )
{
  ...
  if (IS_WOLFSHAPE(ch)) {
    send_to_char("You can't wear, wield or hold while being a wolf.\n\r", ch);
    return;
  }
...
*/
    ...
    if (IS_WEAPON_STAT(obj, WEAPON_TWO_HANDS)) {
			if (IS_SET(ch->flaws, FLAW_SHORT)) {
				send_to_char ("That weapon is too large for someone of your height!\n\r",ch);
				return;
				}
...
/*
  if (!IS_FORSAKEN(ch)) {
    if (obj->item_type == ITEM_ANGREAL && !can_restring_angreal(ch)) {
	 sprintf(buf, "%s takes %s and study it for a moment shaking his head slowly as he says, '{7I can't help you with this!{x'", PERS(mob, ch), obj->short_descr);
	 send_to_char(buf, ch);
	 return;
    }
  } 
...
void do_sheath( CHAR_DATA *ch, char *argument )
{
   ...
   */
   ...
   if (get_skill(ch, gsn_blademaster) > 0 && obj_wield->value[0] == WEAPON_SWORD && IS_SET(ch->auto_act, AUTO_MASTERFORMS)) {
	act("You swing $p smoothly around from guard stance and sheath it in a fluid motion.", ch,obj_wield,NULL,TO_CHAR);
	act("$n swings $p smoothly around from guard stance and sheaths it in a fluid motion.", ch,obj_wield,NULL,TO_ROOM);
   }
   // if Speardancer
	...
	act("$n twirls $p around in a flurry for a moment before $e suddenly sheaths it.", ch,obj_wield,NULL,TO_ROOM);
   }
   // If Duelling
	...
	act("$n twirls $p around very fast before $e suddenly sheaths it.", ch,obj_wield,NULL,TO_ROOM);
   }   
   // If Staffmaster
   else if (get_skill(ch, gsn_staffmaster) > 0 && obj_wield->value[0] == WEAPON_STAFF && IS_SET(ch->auto_act, AUTO_MASTERFORMS)) {
	act("You grasp $p and twirl it masterfully before sheathing it.", ch,obj_wield,NULL,TO_CHAR);
	act("$n grasps $p and twirls it masterfully before sheathing it.", ch,obj_wield,NULL,TO_ROOM);
   }   
   // If Whipmaster
   else if (get_skill(ch, gsn_whipmaster) > 0 && obj_wield->value[0] == WEAPON_WHIP && IS_SET(ch->auto_act, AUTO_MASTERFORMS)) {
	act("You snap $p back to your hand and hang it on your belt.", ch,obj_wield,NULL,TO_CHAR);
	act("$n snaps $p back to their hand and hangs it on their belt.", ch,obj_wield,NULL,TO_ROOM);
   }   
   // If Axemaster
   else if (get_skill(ch, gsn_axemaster) > 0 && obj_wield->value[0] == WEAPON_AXE && IS_SET(ch->auto_act, AUTO_MASTERFORMS)) {
	act("You throw $p into the air and lean forward, sheathing it as it lands perfectly.", ch,obj_wield,NULL,TO_CHAR);
	act("$n throws $p into the air and leans forward, sheathing it as it lands perfectly.", ch,obj_wield,NULL,TO_ROOM);
   }
   // If Flailmaster
   else if (get_skill(ch, gsn_flailmaster) > 0 && obj_wield->value[0] == WEAPON_FLAIL && IS_SET(ch->auto_act, AUTO_MASTERFORMS)) {
	act("You slam $p onto the ground before sheathing it.", ch,obj_wield,NULL,TO_CHAR);
	act("$n slams $p onto the ground before sheathing it.", ch,obj_wield,NULL,TO_ROOM);
   }
   // All other
   else {
...
void do_draw( CHAR_DATA *ch, char *argument )
{
   ...
   obj_wield = get_eq_char(ch, WEAR_WIELD);
   
   	if (!str_prefix(arg1, "arrow"))
	{
		/* Added so players can have hip quivers, too.
			 * Should be beneficial when switch from Bow
			 * to Missile code. 			  - Phil
			 */
		...
		if (arg2[0] == '\0')
		{
			if (obj_wield == NULL) 
			{
				send_to_char("You aren't wielding a bow.  Please specify what you want to draw.\n\r",ch);
				return;
				}
			if ((quiver = get_eq_char(ch, WEAR_BACK)) == NULL ||
				quiver->item_type != ITEM_QUIVER)
			{
				if ((quiver = get_eq_char(ch, WEAR_WAIST)) == NULL ||
					quiver->item_type != ITEM_QUIVER)
				{
				...
				if (arg2[0] != '\0')
		{
			if ((quiver = get_obj_wear(ch, arg2)) == NULL)
			{
				act("You are not wearing $T.",ch,NULL,arg2,TO_CHAR);
				return;
			}
		}
		...
		if (quiver->value[0] < 1)
		{
			send_to_char("Your quiver is now out of arrows, you need to find another one.\n\r",ch);
			return;
		}	
		...
		if (quiver->value[0] > 0)
		{
			...
			if (quiver->value[0] == 2)
			{
				send_to_char("{RBe wary, you have one arrow left in there.{x\n\r",
					ch);
			}
   ...
   // Check and set up defaults for autodraw
   if (obj_s1 != NULL) {
      scabbard = WEAR_SCABBARD_1;
      obj = obj_s1;
   }
  ...
  if (IS_AFFECTED(ch, AFF_WRAPPED)) {
   ...
   if (!IS_NULLSTR(arg1)) {
      if ((obj_s1 != NULL) && is_name( arg1, obj_s1->name)) {
      	 scabbard = WEAR_SCABBARD_1;
         obj = obj_s1;
      }
      else if ((obj_s2 != NULL) && is_name( arg1, obj_s2->name)) {
      	scabbard = WEAR_SCABBARD_2;
         obj = obj_s2;
      }
      else {
   ...
   if (wear == WEAR_SECOND_WIELD) {
     if ( !IS_NPC(ch) &&  ch->level < skill_table[gsn_dual_wield].skill_level[ch->class] ) {
        send_to_char("You are wielding a weapon already.\n\r", ch);
        return;
     }
   ...
   if (get_skill(ch, gsn_blademaster) > 0 && obj->value[0] == WEAPON_SWORD && IS_SET(ch->auto_act, AUTO_MASTERFORMS)) {
	act("You lean slightly forward and smoothly draw $p in a fluid motion.", ch,obj,NULL,TO_CHAR);
	act("$n leans slightly forward and smoothly draws $p in a fluid motion.", ch,obj,NULL,TO_ROOM);
   }
	...
	act("$n jumps into a backflip and draws $p as they spin around to their feet.", ch,obj,NULL,TO_ROOM);
   }
	...
	act("$n moves their arm slightly and draws $p, twirling it around in their hand for a moment!", ch,obj,NULL,TO_ROOM);  
   }
   else if (get_skill(ch, gsn_staffmaster) > 0 && obj->value[0] == WEAPON_STAFF && IS_SET(ch->auto_act, AUTO_MASTERFORMS)) {
	act("You grasp $p and twirl it around before going into a ready position!", ch,obj,NULL,TO_CHAR);
	act("$n grasps $p and twirls it masterfully before going into a defensive stance!", ch,obj,NULL,TO_ROOM);
   }   
   else if (get_skill(ch, gsn_whipmaster) > 0 && obj->value[0] == WEAPON_WHIP && IS_SET(ch->auto_act, AUTO_MASTERFORMS)) {
	act("You grab $p and unfurl it with a quick {Ysnap{x!", ch,obj,NULL,TO_CHAR);
	act("$n grabs $p and unfurls it with a quick {Ysnap{x!", ch,obj,NULL,TO_ROOM);
   }  
   else if (get_skill(ch, gsn_axemaster) > 0 && obj->value[0] == WEAPON_AXE && IS_SET(ch->auto_act, AUTO_MASTERFORMS)) {
	act("You grab $p and twirl it in your hands before holding it out in front of you.", ch,obj,NULL,TO_CHAR);
	act("$n grabs $p and twirls it in their hands before holding it in front of them.", ch,obj,NULL,TO_ROOM);
   }  
   else if (get_skill(ch, gsn_flailmaster) > 0 && obj->value[0] == WEAPON_FLAIL && IS_SET(ch->auto_act, AUTO_MASTERFORMS)) {
	act("You unhook $p and begin to spin it around!", ch,obj,NULL,TO_CHAR);
	act("$n unhooks $p and begins to spin it around!", ch,obj,NULL,TO_ROOM);
   }
   else {
...
void do_repair(CHAR_DATA *ch, char *argument)
{
  ...
  else
  {
     ...
     if ((obj->condition >= 100) && (!IS_SET(obj->extra_flags,ITEM_BROKEN))) {
...
void do_estimate(CHAR_DATA *ch, char *argument)
{
  ...
  if ((obj = (get_obj_carry(ch, arg, ch))) == NULL) {
    act("$N looks at you for a moment then says '{7You don't have that item.{x'",ch,NULL,mob,TO_CHAR);
    return;
  }
...
void do_mend (CHAR_DATA *ch, char *argument)
{
	...
	one_argument(argument,arg);
	fail -= (get_skill(ch,gsn_mend) / 10);
   
   
   if (IS_NULLSTR(arg)) {
	send_to_char("What would you like to mend?\n\r",ch);
	return;
	}
    ...
    if ((obj->condition == 100) && (!IS_SET(obj->extra_flags,ITEM_BROKEN))) {
       send_to_char("But that item is not damaged.\n\r",ch);
       return;
     }
	...
	if (number_percent() < fail) {
		act("You fail miserably at your attempt to mend $p and destroy it.\n\r",ch,obj,NULL,TO_CHAR);
		act("$n fails miserably at the attempt to mend $p and has destroyed it.\n\r",ch,obj,NULL,TO_ROOM);
		check_improve(ch,gsn_mend,FALSE,1);
		return;
		}
	else	
	{
		act("You mend $p and make it as good as new.",ch,obj,NULL,TO_CHAR);
		REMOVE_BIT(obj->extra_flags, ITEM_BROKEN);
		obj->condition = 100;
...
void do_swap (CHAR_DATA *ch, char *argument)
{
  ...
  wield2 = get_eq_char(ch,WEAR_SECOND_WIELD);
   ...
   if (IS_WOLFSHAPE(ch)) {
    send_to_char("You are not wielding anything that you could swap from hand to hand.\n\r", ch);
    return;
  }
 ...
 if (get_eq_char(ch,WEAR_SECOND_WIELD) != NULL) {
	   if (IS_SET(wield1->extra_flags,ITEM_NOREMOVE) && !IS_SET(wield2->extra_flags,ITEM_NOREMOVE))
		{
			REMOVE_BIT(wield1->extra_flags,ITEM_NOREMOVE);
			remove_obj(ch, WEAR_SECOND_WIELD,TRUE);
		...
		if (IS_SET(wield2->extra_flags,ITEM_NOREMOVE) && !IS_SET(wield1->extra_flags,ITEM_NOREMOVE))
		{
			REMOVE_BIT(wield2->extra_flags,ITEM_NOREMOVE);
			remove_obj(ch, WEAR_SECOND_WIELD,TRUE);
			remove_obj(ch, WEAR_WIELD,TRUE);
	...
	send_to_char("You have to be wielding two weapons to be able to swap them around.\n\r",ch);
	}
}
void do_tinker( CHAR_DATA *ch, char *argument )
{
  ...
  if (ch->race == race_lookup("tuathan"))  {
  
  if ( type[0] == '\0' || arg1[0] == '\0' || arg2[0] == '\0' ) {
    send_to_char("Syntax:\n\r",ch);
    send_to_char("  tinker <name> <field> <string>\n\r",ch);
    send_to_char("    fields: name short long \n\r",ch);
    return;
  }
        /* string an obj */
  ...
  send_to_char("You must be a Tinker in order to do this.\n\r",ch);
  }
}
